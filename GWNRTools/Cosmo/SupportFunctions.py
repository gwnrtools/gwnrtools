...
def make_z_cosmo_inverseCDF(z_max, R0, H0, Omega_m, Omega_Lambda, Omega_k, w0, w1):
    z_arr = np.linspace(1e-8,z_max,int(1e5))
    dz = z_arr[1] - z_arr[0]
    R_0 = R0*np.power(1e9,-3)/np.power(1e6,-3)
    R_zmax = integrate.quad(dR_dz, 0, z_arr[-1], (H0, Omega_m, Omega_Lambda, Omega_k, w0, w1, R_0))[0]

    prob_z = np.zeros_like(z_arr)
    for i in xrange(z_arr.shape[0]):
        prob_z[i] = probability_density_Uniform_comoving_volume(z_arr[i], H0, \
                                                            Omega_m, Omega_Lambda, Omega_k, \
                                                            w0, w1, R_0, R_zmax)
...
def probability_density_Uniform_comoving_volume(z, H0, Omega_m, Omega_Lambda, Omega_k, w0, w1, r0, R_zmax):
    #eqn 11 in dP_L_M
    # NSBH rate 3210  Gpc^-3 yr^-1, for aligned spin BHs
    #R_zmax = 3210*np.power(1e9,-3)/np.power(1e6,-3) # Mpc^-3 yr^-1
    return dR_dz(z, H0, Omega_m, Omega_Lambda, Omega_k, w0, w1, r0)*(1/R_zmax)

def dR_dz(z, H0, Omega_m, Omega_Lambda, Omega_k, w0, w1, r0):
    #eqn 12 in dP_L_M
    e_of_z = 1.#(z, PARAMETERS) #assume this is constant across the

    return dV_dz(z, H0, Omega_m, Omega_Lambda, Omega_k, w0, w1)*r0*e_of_z/(1+z)
...
def H(z, H0, Omega_m, Omega_Lambda, Omega_k, w0, w1):
    return H0*np.sqrt((Omega_m*np.power(1+z,3)) + (Omega_k*np.power(1+z,2)) + (Omega_Lambda*E(z,w0,w1)))

def OneOverH(z, H0, Omega_m, Omega_Lambda, Omega_k, w0, w1):
    return 1./(H0*np.sqrt((Omega_m*np.power(1+z,3)) + (Omega_k*np.power(1+z,2)) + (Omega_Lambda*E(z,w0,w1))))
...
def DL(z, H0, Omega_m, Omega_Lambda, Omega_k, w0, w1):
    ...
    if Omega_k > 0.:
        return ((lal.C_SI*1e-3)*(1+z)/np.sqrt(Omega_k))*np.sinh(np.sqrt(Omega_k)*\
                                                Hubble_integral(z, H0, Omega_m, Omega_Lambda, Omega_k, w0, w1))
    elif Omega_k == 0.:
        #print 'Omega_k == 0.'
        return (lal.C_SI*1e-3)*(1+z)*Hubble_integral(z, H0, Omega_m, Omega_Lambda, Omega_k, w0, w1)

    else:
        return ((lal.C_SI*1e-3)*(1+z)/np.sqrt(np.absolute(Omega_k)))*np.sin(np.sqrt(np.absolute(Omega_k))*\
                                                Hubble_integral(z, H0, Omega_m, Omega_Lambda, Omega_k, w0, w1))

def DL_vector(z_arr, H0, Omega_m, Omega_Lambda, Omega_k, w0, w1):
    DL_arr = np.array([DL(z, H0, Omega_m, Omega_Lambda, Omega_k, w0, w1) for z in z_arr])
    return DL_arr

def Hubble_integral(z_prime, H0, Omega_m, Omega_Lambda, Omega_k, w0, w1):
    return integrate.quad(OneOverH, 0, z_prime, (H0, Omega_m, Omega_Lambda, Omega_k, w0, w1))[0]
