...
IN PROGRESS: Adding facility to use "FromDataFile" waveforms
    ...
    if aligned_spin_tmplt_only:
        print "WARNING: Spin components parallel to L allowed, others set to 0 in templates."
    ...
    if signal_approx in pywf.fd_approximants():
        ...
        #try:
        if True:
            if verbose: print ".. generating signal waveform "
    ...
    else: raise IOError("Signal Approximant %s not found.." % signal_approx)
    if verbose:
        print >>sys.stdout, \
    ...
    if signal_approx in pywf.fd_approximants():
        ...
        if 'SEOBNRv4_ROM' in signal_approx or 'SEOBNRv2_ROM' in signal_approx:
            signal_h = extend_waveform_FrequencySeries(signal['H1'], filter_n, force_fit=True)
        else:
    ...
    else: raise IOError("Template Approximant %s not found.." % tmplt_approx)
    #
    if aligned_spin_tmplt_only:
    ...
    if tmplt_approx in pywf.fd_approximants():
        ...
        if 'SEOBNRv4_ROM' in tmplt_approx or 'SEOBNRv2_ROM' in tmplt_approx:
            template_h = extend_waveform_FrequencySeries(template['H1'], filter_n, force_fit=True)
        else:
            template_h = extend_waveform_FrequencySeries(template['H1'], filter_n)
    elif tmplt_approx in pywf.td_approximants():
        try:
            template = generator.generate_from_args(_m1, _m2, _s1x, _s1y, _s1z, _s2x, _s2y, _s2z,\
                                  phic, tc, ra, dec, polarization)
...
IN PROGRESS: Adding facility to use "FromDataFile" waveforms
    ...
    if vary_masses_only and vary_masses_and_aligned_spin_only:
        raise IOError("Inconsistent options: vary_masses_only and vary_masses_and_aligned_spin_only")
    ...
    delta_f  = 1./signal_duration
    if verbose:
        print "signal_duration = %d, sample_rate = %d, filter_N = %d, filter_n = %d" % (signal_duration, sample_rate, filter_N, filter_n)
    ...
    if signal_approx in pywf.fd_approximants():
        ...
        if True:
            ...
            signal_h = extend_waveform_FrequencySeries(signal_h, filter_n)
        #except: raise IOError("Approximant %s not found.." % signal_approx)
    else: raise IOError("Approximant %s not found.." % signal_approx)
    ...
    if tmplt_approx in pywf.fd_approximants():
        ...
        raise IOError("Time-domain templates not supported yet (TDomainDetFrameGenerator doesn't exist)")
        ...
        raise RuntimeError("Using **templates** from data files is not implemented yet")
    else: raise IOError("Approximant %s not found.." % tmplt_approx)
    ...
    def objective_function_fitting_factor(x, *args):
        ...
        tmplt   = tmplt_generator.generate_from_args(m1, m2, _s1x, _s1y, _s1z, _s2x, _s2y, _s2z)
        tmplt_h = make_frequency_series( tmplt['H1'] )

        if debug:
            print "IN FF Objective-> for parameters:",  m1, m2, _s1x, _s1y, _s1z, _s2x, _s2y, _s2z
        if debug:
            print "IN FF Objective-> Length(tmplt) = %d, making it %d" % (len(tmplt['H1']), filter_n)
        ## NOTE: SEOBNRv4 has extra high frequency content, it seems..
        if 'SEOBNRv4_ROM' in tmplt_approx or 'SEOBNRv2_ROM' in tmplt_approx:
        ...
        if debug:
            print "MATCH IS %.6f for parameters:" % m, m1, m2, _s1x, _s1y, _s1z, _s2x, _s2y, _s2z

        retval = np.log10(1. - m)

        # We do not want PSO to go berserk, so we stop when FF = 0.999999
        if retval <= -6.0: retval = -6.0
        return retval
    ...
    if verbose:
        print m1, m2, mt, et, mc, mc_min, mc_max, et_min, et_max, m1_min, m1_max, m2_min, m2_max
    ...
    if verbose:
        print >>sys.stdout,\
            "Overlap with aligned_spin_tmplt_only = ", vary_masses_and_aligned_spin_only, \
            " and non_spin_tmplt_only = ", vary_masses_only, ": ", olap, np.log10(1. - olap)
        sys.stdout.flush()
    #
    idx = 1
    ff = 0.0
    while ff < olap:
        ...
        if verbose:
            ...
            sys.stdout.flush()
        params, ff =\
            pso(objective_function_fitting_factor,
                low_lim, high_lim,
    ...
    if verbose:
        print >>sys.stdout,\
            "optimization took %d objective func evals" % objective_function_fitting_factor.counter
        sys.stdout.flush()
    #
