...
import pylab
from pylab import arange,pi,sin,cos,sqrt,cm
import qm
import numpy
...
### option parsing ###
parser = OptionParser()
parser.add_option('-t', '--tmplt-bank-file', metavar='FILE', help='file containing template to plot', type=str)
...
for n in range (options.num):
  ...
  for injection in sim_table:
    ...
    t0,t3=qm.m1_m2_to_tau0_tau3(injection.mass2*qm.QM_MTSUN_SI,injection.mass1*qm.QM_MTSUN_SI,40)  
    tau0.append(t0)
    tau3.append(t3)
...
for template in sngl_inspiral_table:
  t0,t3=qm.m1_m2_to_tau0_tau3(template.mass2*qm.QM_MTSUN_SI,template.mass1*qm.QM_MTSUN_SI,40)  
  ttau0.append(t0)
  ttau3.append(t3)
...
pylab.hist(matches, bins = nbins, normed=True, cumulative=True, log=True, color='blue',alpha=0.6)
pylab.grid()
pylab.xlim(xmax=1.)
pylab.xlabel("FF (X)")
...
colormap = matplotlib.mlab.griddata( q, chi1, matches, qmap, chi1map)#[(1.-x) for x in matches], qmap, chi1map)
...
pylab.contourf( qmap, chi1map, colormap, [.93,.95,.965,.97,.98,.99,1.])#, cmap=matplotlib.colors.LinearSegmentedColormap('my_colormap',cm.datad.get('hot_r'),256) )
pylab.hold(True)
pylab.plot(tmpltq, tmpltch, 'kx', markersize=3.)
