...
for d in dirs:
  for ld in subdirs[d]:
    try: os.chdir('%s/%s' % (d,ld))
    except: continue
    print "\n\nInside %s" % cmd.getoutput('pwd')
    out_files = cmd.getoutput('ls *.o????*').split()
    testn = 0
    for f in out_files:
      testn = max(testn, len(cmd.getoutput('cat %s | grep "Requested\ timestep\ not\ in\ worldtube\ data\ file.\ Stopping"' % f).split()))
    print "Found ", testn
    if testn != 0: print "This has finished!"
    ...
    try:
      ...
      if qq != 1: print "Warning: This run might not have stopped yet!"
      else: print "This run is stopped. Continuation needed."
    except: print "Unable to determine run status. continue."
    ...
    cmd.getoutput('cp /home/p/pfeiffer/prayush/scratch/projects/CCE_modeldir/highResCce700-resubmit.par highResCce.par')
    #
...
class cce_run():
  ...
  def get_recent_output_file(self, ld):
    ...
    for ff in ofiles:
      ...
      if mtime > max_mtime: max_mtime, ofile = mtime, ff
    os.chdir(pwd)
    return ofile
  #
  def is_to_be_continued(self, ld):
    ...
    if usedtime < askedtime:
      if usedtime < 120:
        if self.verbose:
          print >>sys.stdout, \
            "Warning: %s/%s Run stopped too soon!" % (self.dirname,ld)
        return False
      else:
  ...
  def setup_rerun(self, ld):
    ...
    nn = len(glob.glob('highResCce-*.par')) + 1
    #
    if self.verbose: print "Moving highResCce.par to highResCce-%d.par" % nn
    cmd.getoutput('mv highResCce.par highResCce-%d.par' % nn)
    #
    if self.verbose:
      print "Copying highResCce-resubmit.par to highResCce.par"
    cmd.getoutput('cp /home/p/pfeiffer/prayush/scratch/projects/CCE_modeldir/highResCce700-resubmit.par highResCce.par')
    #
    if self.verbose: print "Moving highResCce to highResCce-%d" % nn
    cmd.getoutput('mv highResCce highResCce-%d' % nn)
    cmd.getoutput('rm -rf highResCce-0')
    cmd.getoutput('ln -s highResCce-%d highResCce-0' % nn)
    os.chdir(pwd)
  ...
  def submit_rerun(self, ld):
    pwd = cmd.getoutput('pwd')
    os.chdir('%s/%s/%s' % (self.workdir,self.dirname,ld))
    if self.verbose: print "Submitting new job!\n\n"
    comm = 'qsub -d `pwd` -N %s_%s_%d -l nodes=1:ppn=8,walltime=48:00:00 ./Submit.input >> sub.out' % (self.dirname,ld,nn)
    cmd.getoutput(comm)
    os.chdir(pwd)
  #}}}
