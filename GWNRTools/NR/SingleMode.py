...
Takes in the new sampling time step, in units of total mass M
        ...
        if delta_t != self.delta_t or not hasattr(self, "mode_array"):
            ...
            mode_array   = self.mode_real_interp(t_array) + self.mode_imag_interp(t_array) * 1.0j
            self.mode_array = TimeSeries(mode_array, delta_t=delta_t, copy=True)
            find_max_start  = len(self.mode_array) * 4 / 5
            max_idx         = find_max_start + self.mode_array[find_max_start:].abs_max_loc()[-1]
            if self.verbose > 1:
            ...
            self.mode_array = TimeSeries(self.mode_array,\
                                    epoch=lal.LIGOTimeGPS(-1.*self.mode_array.sample_times[max_idx]),\
                                    copy=True)
        self.dimLess   = True
        self.totalmass = None
        self.distance  = None
