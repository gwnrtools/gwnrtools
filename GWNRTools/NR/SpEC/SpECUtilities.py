...
operation is irreversible and cumulative!
        ...
        for tl in self.pvdlines:
            if re.findall('timestep="\d+.\d+"', tl) != []:
                ...
                #print t_prev, t_curr
                if not near_equal(t_prev, t_curr): cnt += 1
                #print cnt % downsample_factor
                if cnt % downsample_factor == 0: newpvdlines.append(tl)
                t_prev = t_curr
            else: newpvdlines.append(tl)
...
def JOfOmegaNonSpinning(m, eta, om):
    ...
    J5 = (15309./256. + eta * j5 + (9976./105. + 1312.*eta/15.)*eta*np.log(x)) * x**5
...
def near_equal(n1, n2, tol=1e-9):
    if np.abs(n1 - n2) / np.abs(n1 + n2) < tol: return True
    else: return False


########################################
## SPEC FUNCTIONS -- CORE FUNCS
########################################
...
Can deal only with lines indexed in 4 or fewer digits
    ...
    if mm is None: return None, None
    col = int(mm.group(0).strip(']').strip('[')) - 1 #subtract 1 to start col# from 0
    full_key = line[mm.end():].strip().strip('=').strip()
    return col, full_key

def ParseHeaderForSpECTabularOutputASCII(filename,
...
def GetSegmentDirectories(DIR, LEV, use_non_standard_segments=False,
                    non_standard_prefix='./',
                    verbose=False):
  if use_non_standard_segments:
    return GetNonStandardSegmentDirectories(DIR, LEV,
...
def GetSegmentLettersFromName(NAME):
  ...
  if '_Ringdown' in NAME: retval = 'RD' + retval
  return retval


def GetNonStandardSegmentDirectories(DIR, LEV,
                            non_standard_prefix='LATE_RINGDOWN_START_SEGMENTS/',
                            verbose=False, debug=False):
  ##{{{
  standard_inspiral_dirs = sorted(glob.glob(os.path.join(DIR, 'Lev%d_??/') % LEV))
  standard_ringdown_dirs = sorted(glob.glob(os.path.join(DIR, 'Lev%d_Ringdown/Lev%d_??/' % (LEV, LEV))))
  ##
  inspiral_dirs = sorted(glob.glob(os.path.join(DIR, non_standard_prefix+'Lev%d_??/') % LEV))
...
directory. Returns combined data in a numpy.array.
  """
  #{{{
  if not os.path.exists(DIR): raise IOError("%s does not exist" % DIR)
  #
  inspiral_dirs = GetSegmentDirectories(DIR, LEV,
...
Inputs needed are the main run directory (path to "Ev"), and Lev number (int)
    """
    ahc_glob = glob.glob(os.path.join(DIR,
...
def GetOpOfQuantityOverDomain(data_dict, op_func = DummyMin,
...
op_func   : (function) It takes in all {Q_i} together and maps
    ...
    for idx, tvalue in enumerate(all_tseries):
        ...
        for sd in all_subdomains:
            sd_tseries = data_dict[sd][:,0]
            if tvalue not in sd_tseries: continue
        ...
        subdomainseries.append( avail_subdomains[ np.where(op_func(avail_values,avail_subdomains) == yseries[-1])[0][0] ] )
