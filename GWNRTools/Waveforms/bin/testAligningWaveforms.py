...
def align_waveforms_optimally(h_plus1, h_cross1, hplus2, hcross2, psd=None,\
  ...
  #
  hp2, hc2 = \
    shift_waveform_phase_time(h_plus2, h_cross2, tsign*t_shift, phsign*ph_shift)
...
psd = from_txt('/home/prayush/research/advLIGO_PSDs/ZERO_DET_high_P.dat', N, 1./T, f_low)
...
if True:
  ...
  hc1 = TimeSeries( -1 * np.sin(tvec * 2 * np.pi /  T), delta_t = 1./sample_rate )

orig_length = len(hp1)

h = TimeSeries( np.zeros(N) , delta_t=hp1.delta_t, epoch=hp1._epoch, dtype=real_same_precision_as(hp1) )
h[:len(hp1)] = hp1
hp1 = h
h = TimeSeries( np.zeros(N) , delta_t=hc1.delta_t, epoch=hc1._epoch, dtype=real_same_precision_as(hc1) )
h[:len(hc1)] = hc1
hc1 = h
...
if True:
  ...
  hc2 = TimeSeries( -1 * np.sin(tvec * 2 * np.pi /  T), delta_t = 1./sample_rate )

orig_length = max(orig_length, len(hp2))

h = TimeSeries( np.zeros(N) , delta_t=hp2.delta_t, epoch=hp2._epoch, dtype=real_same_precision_as(hp2) )
h[:len(hp2)] = hp2
hp2 = h
h = TimeSeries( np.zeros(N) , delta_t=hc2.delta_t, epoch=hc2._epoch, dtype=real_same_precision_as(hc2) )
h[:len(hc2)] = hc2
hc2 = h
...
nrwav2 = UseNRinDA.nr_waveform(filename='/home/prayush/research/cita-papers/SEOBtesting/Paper1/plots/InvestigateSimulations/SKS_d14.3-q3-sA_0_0_0.73132_sB_0_0_-0.85/rhOverM_CcePITT_Asymptotic_GeometricUnits.h5', filetype='HDF', time_length=T)
...
nrwav = UseNRinDA.nr_waveform(filename='/home/prayush/research/cita-papers/SEOBtesting/Paper1/plots/InvestigateSimulations/SKS_d14.3-q3-sA_0_0_0.73132_sB_0_0_-0.85/rhOverM_Asymptotic_GeometricUnits.h5', filetype='HDF', cce=False, ex_order=3, time_length=T)
...
for i in range(len(hp1)):
  if hp1[i] == 0 and hc1[i] == 0 and hp1[i+1] == 0 and hc1[i+1] == 0 and hp1[i+2] == 0 and hc1[i+2] == 0:
    break
i1 = i
for i in range(len(hp2)):
  if hp2[i] == 0 and hc2[i] == 0 and hp2[i+1] == 0 and hc2[i+1] == 0 and hp2[i+2] == 0 and hc2[i+2] == 0:
    break
...
snr, corr, snr_norm = matched_filter_core( htilde, stilde, psd, f_low, None, None, out=_snr)
v2_norm = sigma(stilde, psd, f_low, None)

matchCplx = TimeSeries( snr.data  * snr_norm / v2_norm, dtype=complex_same_precision_as(snr), delta_t=hp1.delta_t, epoch=snr._epoch)
...
for i in range(1400, idx_length, int(0.0005/hp1.delta_t)):
  ...
  ax.plot( matchCplx.sample_times, np.abs(matchCplx), 'k', matchCplx.sample_times[i], np.abs(mCplx), 'ro')
  ax.grid()
  ax.set_xlim(0 + np.float64(matchCplx._epoch), idx_length * hp1.delta_t + np.float64(matchCplx._epoch) )
  #
  ax = fig.add_subplot(212)
  ax.plot(hp1.sample_times, hp1, thp2.sample_times, thp2)
  ax.legend(['Extp-N3', 'CCE'], loc='upper left')
  ax.grid()
  ax.set_xlim(0 + np.float64(min(hp1._epoch, thp2._epoch)), idx_length * hp1.delta_t + np.float64(max(hp1._epoch, thp2._epoch)) )
