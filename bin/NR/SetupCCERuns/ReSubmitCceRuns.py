    ...
    try:
      ...
      for ff in ofiles:
        ...
        if mtime > max_mtime:
          max_mtime = mtime
          ofile = ff
      print >>sys.stderr, "Checking outfile = %s" % ofile
      try:
	asked, used = cmd.getoutput('cat %s | grep walltime=' % ofile).split('\n')
        ...
        if usedtime < askedtime:
          ...
          print >>sys.stdout, "Warning: %s/%s run might not have stopped yet!" % (d,ld)
          askuser = input('Should we re-submit this run ([y]/n): ')
	  if 'y' not in askuser: continue
        else: print >>sys.stderr, "%s run is incomplete. Execution time=%d" % (ld,usedtime)
      except: print >>sys.stderr, "Check did not work.Restarting anyway"
    except:
      askuser = input('Check indeterminate. Should we re-submit this run ([y]/n):')
      #if 'y' not in askuser: continue
