...
def get_metadatafiles( nr_tag ):
  levdirs = glob.glob(options.nr_input_dir + '/' + nr_tag + '/Production/BBH*/Lev?')
...
parser.add_option("-x", "--input-catalogs",  help="Names of the xml file to append the information to", default=None )
...
parser.add_option("-Q", "--upper-q-threshold", help='Dont include points with mass-ratio above this threshold', type=float, default=-2.)
...
if options.input_catalogs is not None:
  ...
  if len(input_catalogs) == 0: raise IOError("Please provide catalogs to join")
  for input_catalog in input_catalogs:
    if options.verbose: print "using >> %s" % input_catalog
...
for input_catalog, input_table in input_tables:
  if options.verbose: print >>sys.stderr, "Reading from %s" % input_catalog
  #
  for point in input_table:
    ...
    if point.eta < ( qth / (1. + qth)**2 ):
      if options.verbose:
        print >>sys.stdout, "  -- Not including %s" % point.waveform
        sys.stdout.flush()
      continue
...
if options.verbose:
  print >>sys.stderr, "Total %d points in final table" % len(out_table)
