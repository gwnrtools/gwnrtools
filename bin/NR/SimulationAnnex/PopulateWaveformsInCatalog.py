...
def get_metadatafiles( nr_tag ):
  ...
  for levdir in levdirs:
    ...
    try: metadatafiles[levname] = open(levdir + '/metadata.txt', 'r')
    except: continue
...
first waveform found on disk
  """
  if len(wavetypes) == 0: wavetypes = ['dummy']
  for idx, wavetype in enumerate(wavetypes):
    if wavename != '': filename = wavename
    elif wavetype == 'cce': filename = cce_filename
    elif wavetype == 'extrapolated': filename = extrapolated_filename
    elif wavetype == 'finite-radius': filename = finite_radii_filename
    else: raise RuntimeError("Couldn't find which NR wave file to read.")
    #
    tag = p.waveform
    if options.verbose:
      print >>sys.stdout, "Trying wavetype %s for %s " % (wavetype, tag)
    #
    subdir = tag.split('-')[-1]
    dirname= add_strings(tag.split('-')[:-1], '-')
    workdir = options.nr_input_dir
    # FIXME
    if options.verbose: print tag, subdir, dirname, workdir
    #
    if options.use_hdf:
    ...
    if os.path.exists(h22file) and os.path.getsize(h22file) > 0:
      if options.verbose:
        print >>sys.stdout, "Waveform found for %s at %s" % (tag, h22file)
      return h22file
    elif allow_symlinks and os.path.islink(h22file):
      if options.verbose:
        print >>sys.stdout, "Waveform SYMLINK found for %s at %s" % (tag, h22file)
      return h22file
    else:
      if options.verbose:
        print >>sys.stdout, "Waveform %s NOT found for %s" % (h22file,tag)
  return None
...
parser.add_option("--nr-input-dir", metavar='DIR', help='Main dir with nr sim',default='/home/p/pfeiffer/prayush/scratch/projects/CCE/ChuAlignedSpinning/')
parser.add_option("-i", "--input-tags",  help="Names of the tags" )
parser.add_option("-l", "--lev-tag",  help="RegEx for the Subdirectory of the simulation directory containing the waveform files", default='Production/BBH*/Lev?' )
parser.add_option("-w", "--wavetype",  help="Please provide one of cce, extrapolated, finite-radius", default='' )
parser.add_option("-n", "--wave-name",  help="Please provide one of cce, extrapolated, finite-radius", default='' )


parser.add_option("-x", "--input-catalog",  help="Names of the xml file to append the information to", type=str, default=None )
parser.add_option("-t", "--output-catalog", help='output file name')

parser.add_option("--use-hdf", action="store_true", help="Store ascii file or HDF5 file location?", default=False )
parser.add_option("--use-symlinks", action="store_true", help="Catalog symlinks even if the linked file does not exist", default=False )
parser.add_option("--use-highest-lev", action="store_true", help="Use only the highest lev for each simulation", default=False )

parser.add_option("--restrict-zero-spins", action="store_true", help="Use only the non-spinning waveforms. transverse-spin-threshold is used as the spin-threshold", default=False )
parser.add_option("--restrict-aligned-spins", action="store_true", help="Use only the aligned-spin simulation. transverse spin threshold is reqd", default=False )
parser.add_option("--transverse-spin-threshold", type=float, help="Magnitude of x,y spins below which they are set to 0", default=1.e-4 )
...
if options.input_catalog is not None:
  ...
  for point in input_table:
    ...
    for nn in out_table.columnnames:
      if 'process_id' in nn: npoint.process_id = proc_id
      elif 'waveform' in nn: npoint.waveform = 'NR'
      else: npoint.__setattr__(nn, 0)
    # Copy over columns
    for nn in point.__slots__:
      if hasattr(point, nn): npoint.__setattr__(nn, point.__getattribute__(nn))
    #
    ...
    if npoint.numrel_data == None:
      if options.verbose:
        print >>sys.stderr, "NO WAVE FOUND for %s. SKIPPING.." % npoint.waveform
        sys.stderr.flush()
      del npoint
      continue
    out_table.append( npoint )
  #
  if options.verbose: print >>sys.stderr, len(out_table), " points copied"
else:
  # Construct the table from scratch
  for tag in options.input_tags.split():
    if options.verbose:
      print "\n\n ##################################################"
    if options.verbose: print "checking tag = ", tag
    dont_add = False
    for point in out_table:
      if tag in point.waveform:
        dont_add = True
        if options.verbose: print >>sys.stderr, "This tag already cataloged"
        break
    if dont_add: continue
    mfiles = get_metadatafiles( tag )
    if options.verbose: print "metadatafile = ", mfiles
    mkeys = mfiles.keys()
    ...
    for levname in mkeys:
      ...
      for nn in out_table.columnnames:
        if 'process_id' in nn: npoint.process_id = proc_id
        elif 'waveform' in nn: npoint.waveform = tag + '-' + levname
        elif 'numrel_data' in nn: npoint.numrel_data = 'FILL' # <- CCE hdata
        elif 'numrel_mode_min' in nn: npoint.numrel_mode_min = 2
        elif 'numrel_mode_max' in nn: npoint.numrel_mode_max = 8
        else: npoint.__setattr__(nn, 0)
      #
      if options.verbose:
      ...
      try:
        ...
        del npoint
        continue
      npoint.eta = npoint.mass1*npoint.mass2 / (npoint.mass1+npoint.mass2)**2
      ...
      if npoint.numrel_data == None:
        ...
        if options.verbose:
          print >>sys.stderr,\
            "NO WAVE FOUND for %s. SKIPPING.." % npoint.waveform,\
            npoint.numrel_data
          sys.stderr.flush()
        del npoint
        continue
      ...
      if options.restrict_zero_spins:
        ...
        if abs(npoint.spin1x) > sthreshold or abs(npoint.spin1y) > sthreshold or abs(npoint.spin1z) > sthreshold or abs(npoint.spin2x) > sthreshold or abs(npoint.spin2y) > sthreshold or abs(npoint.spin2z) > sthreshold:
          continue

      if options.restrict_aligned_spins:
        tsthreshold = options.transverse_spin_threshold
        if abs(npoint.spin1x) > tsthreshold or abs(npoint.spin1y) > tsthreshold or abs(npoint.spin2x) > tsthreshold or abs(npoint.spin2y) > tsthreshold:
          continue
        if abs(npoint.spin1z) < tsthreshold and abs(npoint.spin2z) < tsthreshold:
          continue
      ...
      out_table.append( npoint )
      if options.use_highest_lev: break
