...
parser.add_option("--mass1",help="Mass of the heavier object [Msun]",dest="mass1",type=float)
parser.add_option("--mass2",help="Mass of the lighter object [Msun]",dest="mass2",type=float)
parser.add_option("--mchirp-min",help="Lower limit for chirp mass [Msun]",type=float)
parser.add_option("--mchirp-max",help="Upper limit for chirp mass [Msun]",type=float)
parser.add_option("--mtotal-min",help="Lower limit for total mass [Msun]",type=float)
parser.add_option("--mtotal-max",help="Upper limit for total mass [Msun]",type=float)
...
parser.add_option("--ecc-min",help="Lower limit for eccentricity",type=float,default=0)
parser.add_option("--ecc-max",help="Upper limit for eccentricity",type=float,default=0)
parser.add_option("--anomaly-min",help="Lower limit for mean anomaly",type=float,default=0)
parser.add_option("--anomaly-max",help="Upper limit for mean anomaly",type=float,default=0)
parser.add_option("--spin1x",help="s1-x of the heavier object",dest="spin1x",type=float)
parser.add_option("--spin1y",help="s1-y of the heavier object",dest="spin1y",type=float)
parser.add_option("--spin1z",help="s1-z of the heavier object",dest="spin1z",type=float)
parser.add_option("--spin1z-min",help="Max s1-z of the heavier object",type=float, default=-0.99)
parser.add_option("--spin1z-max",help="Min s1-z of the heavier object",type=float, default=0.99)
parser.add_option("--spin1mag",help="s1-magnitude of the heavier object",dest="spin1mag",type=float)
parser.add_option("--spin2x",help="s2-x of the lighter object",dest="spin2x",type=float)
parser.add_option("--spin2y",help="s2-y of the lighter object",dest="spin2y",type=float)
parser.add_option("--spin2z",help="s2-z of the lighter object",dest="spin2z",type=float)
parser.add_option("--spin2z-min",help="Min s2-z of the lighter object",type=float,default=-1.)
parser.add_option("--spin2z-max",help="Max s2-z of the lighter object",type=float, default=1.)
parser.add_option("--spin2mag",help="s2-magnitude of the lighter object",dest="spin2mag",type=float)
parser.add_option("--lambda1-min",help="Lower limit for Lambda of M1",type=float, default=0)
parser.add_option("--lambda1-max",help="Lower limit for Lambda of M1",type=float, default=0)
parser.add_option("--lambda2-min",help="Lower limit for Lambda of M1",type=float, default=0)
parser.add_option("--lambda2-max",help="Lower limit for Lambda of M1",type=float, default=4500)
...
parser.add_option("--inclination",help="inclination angle, between the orbital angular momentum and line of sight from the detector",dest="inclination",type=float)
parser.add_option("--polarization",help="polarization angle, between the radiatoin frame and the detector frame",dest="polarization",type=float)
parser.add_option("--num-points",help="Number of points needed in the test-points file",dest="npoints",type=int)
parser.add_option("--sample-m1-m2",action="store_true",help="pass if you want to sample the m1-m2 axes",default=True)
parser.add_option("--sample-mchirp-eta",action="store_true",help="pass if you want to sample the mchirp-eta axes",default=False)
parser.add_option("--sample-mtotal-q",action="store_true",help="pass if you want to sample the mtotal-q axes",default=False)
...
#
if options.mchirp_max: mchirp_max = options.mchirp_max
else: mchirp_max = mtotal_max * 0.25**0.6
if options.mchirp_min: mchirp_min = options.mchirp_min
else: mchirp_min = (2. * mass_min) * ((mass_max/mass_min)/(1. + (mass_max/mass_min))**2)**0.6
#
if options.mtotal_min: mtotal_min = options.mtotal_min
else: mtotal_min = mass1_min + mass2_min
if options.mtotal_max: mtotal_max = options.mtotal_max
else: mtotal_max = mass1_max + mass2_max

if options.eta_max: eta_max = options.eta_max
else: eta_max = 0.25
if options.eta_min: eta_min = options.eta_min
else: eta_min = (mass_max/mass_min)/(1. + (mass_max/mass_min))**2
#
...
# sampled.
sim_inspiral_table = lsctables.New(lsctables.SimInspiralTable,columns=['mass1','mass2','mchirp','eta','spin1x','spin1y','spin1z','spin2x','spin2y','spin2z', 'inclination','polarization','latitude','longitude','bandpass','alpha','alpha1','alpha2', 'alpha3', 'alpha4', 'process_id'])
...
def accept_point_mchirp_eta( mchirp, eta ):
	...
	if ((m1+m2) <= mtotal_max) and (m1 > mass_min) and (m2 > mass_min) and (m1 < mass_max) and (m2 < mass_max):
		return True
	else:
		return False
...
def accept_point_mtotal_q( mtotal, q ):
	...
	m1, m2 = mchirp_eta_to_m1_m2( mchirp, eta )
	if (mtotal <= mtotal_max) and (mtotal >= mtotal_min) and (eta >= eta_min) and (eta <= eta_max) and (m1 > mass1_min) and (m2 > mass2_min) and (m1 < mass1_max) and (m2 < mass2_max):
		return True
	else:
...
for i in np.arange(npoints):
  if i%1000 == 0 and options.verbose: print >>sys.stderr, "Point %d" % i
  ...
  if options.sample_m1_m2:
	...
	smplpt.mchirp,smplpt.eta = m1_m2_to_mchirp_eta(smplpt.mass1,smplpt.mass2)
  ...
  if options.sample_mchirp_eta:
	...
	smplpt.eta = tempet
	smplpt.mass1, smplpt.mass2 = mchirp_eta_to_m1_m2( smplpt.mchirp, smplpt.eta )
  ...
  if (smplpt.spin1x or smplpt.spin1y or smplpt.spin1z) and not (options.spin1x and options.spin1y and options.spin1z):
    s1mag = np.sqrt( smplpt.spin1x**2.0 + smplpt.spin1y**2.0 + smplpt.spin1z**2.0 )
  ...
  if (smplpt.spin2x or smplpt.spin2y or smplpt.spin2z) and not (options.spin2x and options.spin2y and options.spin2z):
    s2mag = np.sqrt( smplpt.spin2x**2.0 + smplpt.spin2y**2.0 + smplpt.spin2z**2.0 )
    if options.spin2mag:
