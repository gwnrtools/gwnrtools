...
parser.add_option("-n", "--num", help="No of waves to generate", type=int, default=4)
parser.add_option(
    "-o",
    "--output-dir-prefix",
...
parser.add_option("--spin1z-min", help="Min. spin1z", type=float, default=-0.99)
parser.add_option("--spin1z-max", help="Max. spin1z", type=float, default=0.99)
parser.add_option("--spin2z-min", help="Min. spin2z", type=float, default=-0.99)
parser.add_option("--spin2z-max", help="Max. spin2z", type=float, default=0.99)
parser.add_option("--spin-mag-min", help="Min. spin magnitude", type=float, default=0.0)
parser.add_option(
    "--spin-mag-max", help="Max. spin magnitude", type=float, default=0.99
)

parser.add_option("--phi-ref", help="Ref phase [rads]", type=float, default=0)
parser.add_option(
    "--distance", help="Distance to source [Mpc]", type=float, default=1.0e6
)

parser.add_option("--sample-rate", help="Sample Rate [Hz]", type=float, default=4096.0)
parser.add_option(
    "--f-lower", help="Low frequency cutoff [Hz]", type=float, default=14.0
)
...
parser.add_option(
    "--output-type", help="Output type: (0) HDF5 or (1) ASCII", type=int, default=0
)
...
def compress_modes_to_splines(
    ...
    #################################
    t_arr = TimeSeries(
        hLM.tdata.data * total_mass * lal.MTSUN_SI, delta_t=hLM.mode.deltaT
    )
    ...
    if to_amp_phase and not to_modes:
        ...
        ################
        return (
            funcamp_subsampled_spline,
            funcphs_subsampled_spline,
            phsLM_min,
            t_arr.min(),
            t_arr.max(),
        )
        ...
        #
        return (
            func_hreal_subsampled_spline,
            func_himag_subsampled_spline,
            t_arr.min(),
            t_arr.max(),
        )
        #################################
    ...
    for file_name, mode_array, splines, tminmax in waves.values():
        ...
        for jdx, mode_lm in enumerate(mode_array):
            ...
            if modes:
                ...
                phsLM = phase_from_polarizations(spline_real, spline_imag) + np.pi
            ...
            if jdx == 0:
                ...
                tmp_data_array = np.array(list(zip(spline_real.data, spline_imag.data)))
...
def write_raw_modes_to_ASCII(waves, subsamp_n=1, modes=True):
    ...
    for file_name, mode_array, splines, tminmax in waves.values():
        ...
        if not os.path.exists(file_name):
            np.savetxt(
                file_name + ".txt.gz", data_array, fmt=file_format, header=header_string
            )
        else:
...
############# MAIN    # #################
#########################################
filter_dt = 1.0 / options.sample_rate
f_low = options.f_lower

distance = options.distance * lal.PC_SI
phiref = options.phi_ref
inclination = 0 * lal.PI / 6.0  # 0 #0.000189136217684

m2_min = 3.0
q_min = options.q_min
q_max = options.q_max
eta_max = q_min / (1.0 + q_min) ** 2
eta_min = q_max / (1.0 + q_max) ** 2
...
for idx in range(num_waves):
    ...
    try:
        (
            ...
            s2y,
            s2z,
            precessing_eob_version,
        )
        f_samp = 1.0 / filter_dt
    ...
    if options.verbose:
        print("Wave generation+compression done in %.2f seconds" % (time.time() - __t1))

# WRITE THE LAST BATCH
if compress_none:
    if "ASCII" in output_mode:
        write_raw_modes_to_ASCII(waves, subsamp_n=subsamp_n, modes=write_amp_phase)
    elif "HDF" in output_mode:
        write_raw_modes_to_HDF5(waves, subsamp_n=subsamp_n, modes=write_amp_phase)
    else:
        write_raw_modes_to_HDF5(waves, subsamp_n=subsamp_n, modes=write_amp_phase)
        write_raw_modes_to_ASCII(waves, subsamp_n=subsamp_n, modes=write_amp_phase)
else:
