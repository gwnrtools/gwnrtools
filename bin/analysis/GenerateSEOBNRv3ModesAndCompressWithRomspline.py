...
import time
__itime = time.time()
import lal, lalsimulation as ls, os, sys, numpy as np, h5py, commands as cmd
...
if spdeg < 1 or spdeg > 5: raise IOError("degree of spline should be [1,5]")

cflag = options.compression_mode
compress_amplitude_phase = False
compress_modes           = False
compress_none            = False
if cflag == 0: compress_amplitude_phase = True
elif cflag == 1: compress_modes = True
else: compress_none = True
...
if oflag == 0: output_mode = 'HDF'
elif oflag > 0: output_mode= 'ASCII'
else: pass
...
#{{{
    #################################
    t_arr  = TimeSeries(hLM.tdata.data * total_mass * lal.MTSUN_SI, delta_t=hLM.mode.deltaT)
    ...
    if to_amp_phase and not to_modes:
        if verbose: print "Converting to amplitude and phase"
        #################################
        if verbose: __t0 = time.time()
        ...
        if verbose: print "Using mode real/imaginary parts directly"
        #################################
        if verbose: __t0 = time.time()
        ...
        if verbose: print "Using mode real/imaginary parts RAW (NOT COMPRESSING)"
        #################################
        if verbose: __t0 = time.time()
        if verbose:
            print " took %.2f seconds.." % (time.time() - __t0)
...
Write mode time-series to disk as ASCII. Very specific inputs.
  """
#{{{
  for file_name, mode_array, splines, tminmax in waves.itervalues():
...
#{{{
  for file_name, mode_array, splines, tminmax in waves.itervalues():
    ...
    for jdx, mode_lm in enumerate(mode_array):
      ...
      #
      if jdx == 0:
          data_array     = np.array(zip(spline_real.sample_times.data,\
                                  spline_real.data, spline_imag.data))
      elif jdx > 0:
        tmp_data_array = np.array(zip(spline_real.data, spline_imag.data))
...
#{{{
  for file_name, mode_array, splines, tminmax in waves.itervalues():
    ...
    file_format = ''
    for j in range(ncol): file_format += '%.16e\t'
    # Write
    if not os.path.exists(file_name):
      np.savetxt(file_name + '.txt.gz', data_array, fmt=file_format, header=header_string)
    else:
  ...
  if idx != 0 and (idx % num_write == 0):
    if compress_none:
      if 'ASCII' in output_mode:
        ...
        write_raw_modes_to_ASCII(waves, subsamp_n=subsamp_n, modes=write_amp_phase)
        write_raw_modes_to_HDF5(waves, subsamp_n=subsamp_n, modes=write_amp_phase)
    else: write_compressed_modes_to_HDF5(waves)
    del waves
    waves = {}
