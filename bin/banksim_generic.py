...
# IO related inputs
parser.add_argument("--bank-file-name", dest="bank_file_name", help="The bank file")
parser.add_argument(
    "--proposal-file-name", dest="prop_file_name", help="The new points file"
)
parser.add_argument(
    "--match-file-name", dest="match_file_name", help="The file to store matches"
)
...
parser.add_argument(
    "--eccentricity-window", dest="ecc_window", default=0.01, type=float
)
...
def get_waveform(wav, approximant, f_min, dt, N):
    ...
    df = 1.0 / (dt * N)
    f_max = min(1.0 / (2.0 * dt), 0.15 / ((m1 + m2) * lal.MTSUN_SI))

    if approximant in fd_approximants():
        try:
            hptild, hctild = get_fd_waveform(
        ...
        hptilde = FrequencySeries(hptild, delta_f=df, dtype=np.complex128, copy=True)
        hpref_padded = FrequencySeries(
            zeros(N / 2 + 1), delta_f=df, dtype=np.complex128, copy=True
        )
        hpref_padded[0 : len(hptilde)] = hptilde
        hctilde = FrequencySeries(hctild, delta_f=df, dtype=np.complex128, copy=True)
        hcref_padded = FrequencySeries(
            zeros(N / 2 + 1), delta_f=df, dtype=np.complex128, copy=True
        )
...
bank_doc = ligolw_utils.load_filename(
...
n = N / 2 + 1
df = 1.0 / float(signal_length)
if options.verbose:
    logging.info(
        "f_min={}, sig_len={}, sample_rate={}, dt={}, N={}".format(
...
if len(bank_table) == 0:
    ...
    bank_batches = [
        bank_table[i : i + bank_batch_size]
        for i in range(0, len(bank_table), bank_batch_size)
    ]
...
if len(prop_table) == 0:
    ...
    prop_batches = [
        prop_table[i : i + prop_batch_size]
        for i in range(0, len(prop_table), prop_batch_size)
    ]
...
for i, bank_batch in enumerate(bank_batches):
    ...
    if options.verbose:
        logging.info(
            "\t Processing bank batch {} of {} (size {})".format(
                i + 1, len(bank_batches), len(bank_batch)
            )
        )
    for j, prop_batch in enumerate(prop_batches):
        ...
        if options.verbose:
            logging.info(
                "\t\t Processing proposal batch {} of {} (size {})".format(
        ...
        for k, pb in enumerate(bank_batch):
            for l, pp in enumerate(prop_batch):
                ...
                if (
                    ...
                    and outside_mchirp_window(pp, pb, options.mchirp_window)
                    and outside_ecc_window(pp, pb, options.ecc_window)
                ):
                    append_one_match(pb, pp, -1)
                    continue
                ...
                if waveform_exists(pb, bank_batch_waveforms):
                    ...
                    stilde = get_waveform(pb, options.bank_approximant, f_min, dt, N)
                ...
                if waveform_exists(pp, prop_batch_waveforms):
                    ...
                    htilde = get_waveform(
                        pp, options.proposal_approximant, f_min, dt, N
                    )
...
if options.verbose:
    ...
    logging.info(
        "Total {}+{} waves generated, {} matches evaluated.".format(
