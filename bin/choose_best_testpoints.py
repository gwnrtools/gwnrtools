...
# IO related inputs
parser.add_argument(
    "--proposal-file-name", dest="prop_file_name", help="The current points file"
)
parser.add_argument(
    "--old-bank-file-name", dest="old_bank_file_name", help="The old bank file"
)
parser.add_argument(
    "--new-bank-file-name", dest="new_bank_file_name", help="The new points file"
)
parser.add_argument(
    "--match-file-name-glob",
...
def is_eliminated(wav):
    sid = get_tag(wav)
    if os.path.exists(os.path.join(options.elimination_dir, sid)):
        return True
    return False


def parse_match_file(
    mfile_name, mvals_for_each_bank_point={}, mvals_for_each_test_point={}
):
...
prop_doc = ligolw_utils.load_filename(
...
old_bank_doc = ligolw_utils.load_filename(
    options.old_bank_file_name,
    contenthandler=table.use_in(ligolw.LIGOLWContentHandler),
    verbose=options.verbose,
)
try:
...
new_inspiral_table = lsctables.New(
    lsctables.SimInspiralTable, columns=old_bank_table.columnnames
)
outdoc.childNodes[0].appendChild(new_inspiral_table)
out_proc_id = ligolw_process.register_to_xmldoc(
    outdoc, PROGRAM_NAME, options.__dict__, comment=options.comment
).process_id
...
for mfile_name in mfile_names:
    _, mvals_for_each_test_point = parse_match_file(
        mfile_name, {}, mvals_for_each_test_point
    )
# remove self-matches
...
if len(gvals.values()) > 0:
    max_gval = np.max(gvals.values())
else:
    max_gval = -1
if options.verbose:
    logging.info(
        "Init:  G values of {} proposal points compued: {}".format(
...
while max_gval > 0:
    ...
    if options.verbose:
        logging.info(
            "\t maximum G value is {} for {}".format(max_gval, test_point_with_max_gval)
        )
        logging.info(
            "\t\t its G-points include: {}".format(gpoints[test_point_with_max_gval])
        )
    ## 5)
    ## TESTME
    _test = True
    ...
    if _test:
        ...
        for p in points_to_be_removed:
            if p in mvals_for_each_test_point:
                ...
                logging.info("point {} to be removed not found-1Of3!!".format(p))
            if p in gvals:
                gvals.pop(p)
            else:
                logging.info("point {} to be removed not found-2Of3!!".format(p))
            if p in gpoints:
                gpoints.pop(p)
            else:
                logging.info("point {} to be removed not found-3Of3!!".format(p))
        if options.verbose:
            logging.info("\t\t removed {} points.".format(len(points_to_be_removed)))
        for p in gvals:
...
logging.info(
    "Total number of proposal points read = {}".format(len(prop_table_tag_dict))
)
logging.info(
    "Number of surviving proposal points with high G-values = {}".format(
        len(best_testpoints)
    )
)
logging.info(
    "Number of other surviving proposal points with G=0 = {}".format(
...
if options.verbose:
    ...
    logging.info(
        "Total {} test points added, which add up to {} points now.".format(
...
with open("SummaryStatistics.dat", "a+") as fout:
    fout.write(
        ...
        % (
            ...
            len(new_inspiral_table) - len(old_bank_table),
