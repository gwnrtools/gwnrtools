...
model_sample_files['imrpp'] = os.path.join(sample_dir, 'allIMRPPsp_post_evolved_Mfaf_Erad_Lpeak.dat')
model_sample_files['seob'] = os.path.join(sample_dir, 'SEOBNR3_marg_sp_post_evolved_Mfaf_Erad_Lpeak.dat')
comb_sample_file = os.path.join(sample_dir, 'overall_3marg_sp_post_Mfaf_Erad_Lpeak.dat')

# posterior samples for all other files
model_sample_files['imrpp'] = os.path.join(sample_dir, 'allIMRPPsp_post.dat')
model_sample_files['seob'] = os.path.join(sample_dir, 'SEOBNR3_marg_sp_post.dat')
...
# PSD files
psd_files = [os.path.join(sample_dir, 'h1_psd.dat'),
             os.path.join(sample_dir, 'l1_psd.dat')]

# Filtered input
filtered_dir = '../data/OutputFromMakeFigures/'
filtered_time_files = dict([(ifo, os.path.join(filtered_dir, 'Times{}.txt'.format(ifo))) for ifo in ifos])
filtered_strain_files = dict([(ifo, os.path.join(filtered_dir, 'Filtered{}.txt'.format(ifo))) for ifo in ifos])
...
data_residual_files = dict([(ifo, os.path.join(reconstruct_dir, data_file_fmt.format(ifo)))
                            for ifo in ifos])
...
for model in models:
    cbc_files[model] = {}
    for ifo in ifos:
        cbc_files[model][ifo] = os.path.join(reconstruct_dir, cbc_file_fmt.format(model, ifo))
