...
filtered_time_files = dict(
    [(ifo, os.path.join(filtered_dir, "Times{}.txt".format(ifo))) for ifo in ifos]
)
filtered_strain_files = dict(
    [(ifo, os.path.join(filtered_dir, "Filtered{}.txt".format(ifo))) for ifo in ifos]
)
...
data_residual_files = dict(
    [(ifo, os.path.join(reconstruct_dir, data_file_fmt.format(ifo))) for ifo in ifos]
)
...
for model in models:
    cbc_files[model] = {}
    for ifo in ifos:
        cbc_files[model][ifo] = os.path.join(
            reconstruct_dir, cbc_file_fmt.format(model, ifo)
        )
...
white_outline = [patheffects.withStroke(linewidth=2.0, foreground="w")]
