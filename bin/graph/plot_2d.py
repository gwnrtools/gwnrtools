...
def plot_bounded_2d_kde(
    ...
    **kwargs
    ...
    if bivariate == False:
        raise TypeError(
            "Bounded 2D KDE are only available for" "bivariate distributions."
        )
    else:
        ...
        kwargs["linewidths"] = kwargs.get("linewidths", 1.0)
...
def plot_bounded_2d_kde_otherevents(
    data,
    data2=None,
    contour_data=None,
    levels=None,
...
def plot_2d_mass(
    ...
    **kwargs
    ...
    g = sns.JointGrid(
        postHL_series[0], postHL_series[1], xlim=xlim, ylim=ylim, space=0, ratio=ratio
    )  # Need to specify xlim and ylim?
    g = g.plot_joint(
        plot_bounded_2d_kde,
    ...
    plot_bounded_2d_kde_otherevents(
        postHL_series[0],
        postHL_series[1],
        cdata_HL,
        xlim=xlim,
...
def plot_2d_chieff(
    ...
    **kwargs
    ...
    postprior_series = [
        pd.Series(pos_prior[p], name=label) for p, label in zip(params, labels)
    ]
    # postHLV_series = [pd.Series(pos_HLV[p], name=label) for p, label in zip(params, labels)]
...
if plot_m1m2:
    ...
    ax = g.ax_joint
    # ax.xaxis.set_major_formatter(DegreeFormatter())
    ax.set_yticks([15, 20.0, 25.0, 30, 35])
    ax.set_xticks([15, 20.0, 25.0, 30, 35, 40, 45])
    # ax.set_xticks([0., 30., 60., 90,120,150,180])
