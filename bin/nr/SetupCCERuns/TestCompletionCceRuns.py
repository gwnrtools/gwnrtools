...
for d in dirs:
    for ld in subdirs[d]:
        ...
        if abs(tend_data - tend_cce) > 10.0:
            print("NOT finished!!")
        else:
            print("finished")
        os.chdir(pwd)
        ...
        comm = (
            "qsub -d `pwd` -N %s_%s_%d -l nodes=1:ppn=8,walltime=48:00:00 ./Submit.input >> sub.out"
            % (d, ld, nn)
        )
        cmd.getoutput(comm)
        os.chdir(pwd)
...
class cce_run:
    ...
    def __init__(self, workdir=None, dirname=None, verbose=True):
        ...
        self.subdirs = cmd.getoutput("ls %s/%s | grep Lev" % (workdir, dirname)).split()
    ...
    def is_to_be_continued(self, ld):
        pwd = cmd.getoutput("pwd")
        os.chdir("%s/%s/%s" % (self.workdir, self.dirname, ld))
        asked, used = cmd.getoutput("cat %s | grep walltime=" % self.ofiles[ld]).split(
            "\n"
        )
