...
except ImportError: print "Dont uniformly sample output"
...
def overlap_between_waveforms( wav1, wav2, psd=None, f_lower=15. ):
  ...
  if len1 != len2: raise IOError("Length of waveforms not equal")
...
###############################################################################
    ...
    if not post_process_only:
      if datafile is None: raise IOError("Specify CCE data file")
      if datadir is None: raise IOError("Specify CCE data file directory")
      if not os.path.exists("%s/%s" % (datadir, datafile)):
        raise IOError("Input Cce data file cannot be located")
      if not os.path.exists(pittnull): raise IOError("Cannot located pittnull exe")
    if outdir is None: raise IOError("Where to set up the CCE run?")
    #
    self.datafile= datafile
    self.datadir = datadir
    self.outdir  = outdir
    if not os.path.exists(outdir):
      if verbose: print "Outdirectory %s didn't exist..created." % outdir
  ...
  def write_submitfile(self, walltime=24):
    ...
    pwd = cmd.getoutput('pwd')
    os.chdir(self.outdir)
    if walltime > 48 or walltime < 2: walltime = self.walltime
    out = """\
#!/bin/env sh
...
""" %\
  ...
  def write_parfile(self, recover_from=None):
    #{{{
    pwd = cmd.getoutput('pwd')
    if recover_from is None: recover_from = self.prefix + '-0'
    os.chdir(self.outdir)
    out = """\
#------------------------------------------------------------------------------
...
""" % self.ExtractionRadius
    #
    if self.maxtimelevels is None: nullnews_maxtimelevels = 100
    else: nullnews_maxtimelevels = self.maxtimelevels
    out += """
#------------------------------------------------------------------------------
...
""" % (self.get_outdirname(), self.get_parfilename().strip('.par'), \
  ...
  def submit_run(self):
    ...
    if self.verbose: print "Submitting new job: %s\n\n" % comm
    cmd.getoutput('echo %s >> sub.out' % comm)
    out = cmd.getoutput(comm)
    #
    if self.verbose: print >>sys.stderr, out
    if out: print >>sys.stderr, "Make sure the job submission succeeded!"
    #
    os.chdir(pwd)
    return
    #}}}
  ...
  def is_to_be_continued(self, ofile=None, checkforuncopied=False):
    ...
    try:
      if ofile is None and not checkforuncopied:
        ...
        if not os.path.exists(ofile):
          print >>sys.stderr, "Next segment has not written log files. Reverting to the one that has.\n"
          ofile = self.get_recent_error_file()
      else: ofile = ofile
      ...
      if os.path.exists(odirname) and os.path.getsize(odirname) > 1024:
        print >>sys.stdout, "Run probably ongoing, output file not written."
        if os.path.getmtime('%s/Psi4_scri.L02Mp02.asc' % odirname) > \
                      (time.time()-600):
          print >>sys.stdout, "Psi4 files freshly written."
        else:
          print >>sys.stdout, \
        ...
        print "Odirname = ", odirname
        if self.verbose: print >>sys.stdout, "No run ongoing"
      os.chdir(pwd)
      return False
    ##
    #
    ...
    if usedtime < askedtime:
      if usedtime < 120:
        if cmd.getoutput('/bin/cat %s | grep Requested\ timestep\ not\ in\ worldtube\ data\ file' % ofile):
          if self.verbose: 
            print >>sys.stdout, "UseTime = ", usedtime, "\n"
            print >>sys.stdout, "Warning: %s stopped too soon, but the Run might have completed to begin with." % self.outdir
          os.chdir(pwd)
          return False
        elif self.verbose: print >>sys.stdout, "Warning: %s Run stopped too soon!" % (self.outdir)
        os.chdir(pwd)
        return False
    if 'Requested' in cmd.getoutput('/bin/cat %s | grep Requested\ timestep\ not\ in\ worldtube\ data\ file' % ofile) or 'Done' in cmd.getoutput('/bin/cat %s | grep Done' % ofile):
      print "PASSED TEST FOR STRING IN ERR FILE", ofile
      print cmd.getoutput('/bin/cat %s | grep Requested\ timestep\ not\ in\ worldtube\ data\ file' % ofile) != ''
      print cmd.getoutput('/bin/cat %s | grep Done' % ofile)# != ''
      if self.verbose: print >>sys.stdout, "Run %s either Done or requested for timestep not in worldtube data file\n" % (self.outdir)
      if checkforuncopied:
      ...
      print cmd.getoutput('/bin/cat %s | grep Requested\ timestep\ not\ in\ worldtube\ data\ file' % ofile) != ''
      print cmd.getoutput('/bin/cat %s | grep Done' % ofile)# != ''
    if self.verbose: print >>sys.stdout, "Run needs to be restarted"
    os.chdir(pwd)
    return True
      #else:
      #  if self.verbose:
  ...
  def is_to_be_continued_2(self, abs_eps=1.e-10, ofile='Psi4_scri.L02Mp02.asc',\
    ...
    for i in range(1,self.nn):
      ...
      try: end_times.append( psi_d[-1,0] )
      except IndexError:
  ...
  def combine_output(self, subdirs=None, redo=True):
    ...
    if os.path.exists(outdir):
      print >>sys.stderr, "output for %s has been joined already.!" % \
                              cmd.getoutput('pwd')
      if not self.redo:
        os.chdir(pwd)
        return
    ...
    if subdirs is None: subdirs = self.get_all_subdirs()
    if int(subdirs[0][-1]) == 0: subdirs = subdirs[1:]
    if subdirs is None or len(subdirs)==0:
      raise IOError("No directories of the form %s-?. Wrong tag?" % self.prefix)
    #subdirs.sort()
    # Append the most recent portion of the run
    #if os.path.exists(idir+'/highResCce'): subdirs.append(idir+'/highResCce')
    if self.verbose: print >>sys.stderr, "directories used: ", subdirs
    #
    # Join files of each tag
    #
    for ftag in self.filetags:
      #
      # Assume all dirs in subdirs have the same files satisfying the ftag
      tmp_files = glob.glob( subdirs[0] + '/' + ftag )
      file_names = [dd.split('/')[-1] for dd in tmp_files]
      if self.verbose: print >>sys.stderr, "files found: ", file_names
      if self.verbose: print >>sys.stderr, "total: %d" % len(file_names)
      for fnam in file_names:
        outfnam = outdir + '/' + fnam
        if os.path.exists(outfnam) and os.path.getsize(outfnam) and not self.redo:
          print >>sys.stderr, "\nNOT Joining: ", fnam, " in ", subdirs
          continue
        if self.verbose: print >>sys.stderr, "\nJoining: ", fnam, " in ", subdirs
        ...
        # actual data will not be lost by ignoring these trailing lines.
        data = []
        for sdir in subdirs:
          try:
            tmp_data = np.loadtxt(open(sdir + '/' + fnam))
            # It may happen that the output file has only ONE line, handle that!
            if len(np.shape(tmp_data)) < 2: tmp_data = np.array([ tmp_data ])
            data.append( tmp_data )
          except:
            fin = open(sdir + '/' + fnam).readlines()
            tmp_data = []
            for line in fin:
              try: tmp_data.append( np.float128( line.split() ) )
              except: continue
            if not len(tmp_data): data.append( np.array(tmp_data) )
        #
        # Assume the data is in chronological order in the files
        tarr, rearr, imarr = [data[0][:,0], data[0][:,1], data[0][:,2]]
        for dd in data[1:]:
          tmp_t = dd[:,0]
          try: tmp_startidx = np.where( tmp_t > tarr[-1] )[0][0]
        ...
        #
        if self.verbose:
          print >>sys.stderr, "Writing to: %s" % (outdir + '/' + fnam)
        fout = open( outfnam, 'w' )
  ...
  def uniformly_sample_output(self, joineddir=None):
    ...
    # Check if the output directory already exists. Assume output does as well
    # if the directory does
    if joineddir is None: outdir = './' + self.prefix + '.joined'
    else: outdir = joineddir
    if not os.path.exists(outdir):
      print "Outdir is ", outdir
      raise RuntimeError(\
        "The director for combined PSi4 output doesn't exist" % outdir)
    else: os.chdir(outdir)
    #
    for ftag in self.filetags:
      ...
      for fnam in fnames:
        ...
        fout.close()
        #
        if self.verbose: print >>sys.stderr, "Uniformly sampled %s" % fnam
    #
    os.chdir(pwd)
    #}}}
  #
...
using the FFI method from http://arxiv.org/abs/1006.1632
  ...
  HDF : inputdir = ., outdir = .
    """
    #{{{
    try: from scipy.integrate import simps
    except: raise ImportError("Could not import simps from scipy.integrate")
    pwd = cmd.getoutput('pwd')
    os.chdir(self.outdir)
    # Check if the output directory already exists. Assume output does as well
    # if the directory does
    ...
    if self.verbose: print "Integrating modes in %s" % outdir
    if not os.path.exists(outdir):
      raise RuntimeError(\
        "The director for combined PSi4 output doesn't exist" % outdir)
    elif resample:
    ...
    if 'ASCII' in datatype:
      if inputdir is not None: initial_dir = inputdir
      else: initial_dir = outdir
    elif 'HDF' in datatype:
      if inputdir is None: inputdir = '.'
    ...
    for l in range(2,lmax+1):
      for m in range(-l, l+1):
        if m == 0 and m0_time_domain: continue # Handle m=0 modes in time-domain
        if self.verbose: print "Load (l,m) = ", l, m
        WF[l][m] = WaveFunction([], [])
        if 'ASCII' in datatype:
          if fstring is not None: fnam = initial_dir+'/'+fstring % (l,m)
          else:
            if m < 0: mstr = 'm%02d' % abs(m)
            else: mstr = 'p%02d' % abs(m)
            fnam = ("%s/Psi4_scri.L%02dM" % (initial_dir,l)) +\
                                          mstr +"_uform.asc"
          if self.verbose: print "reading %s" % fnam
          WF[l][m].Load(fnam)
        elif 'HDF5' in datatype:
          ccegrp = self.prefix + '.dir'
          WF[l][m].Load( datafin[ccegrp]['Y_l%d_m%d.dat' % (l,m)].value, \
                          datatype='HDFfp' )
        else: raise IOError("datatype must be either ASCII or HDF5")
    ...
    for l in range(2,lmax+1):
      for m in range(-l, l+1):
        if m == 0 and m0_time_domain: continue # Handle m=0 modes in time-domain
        if self.verbose: print "Integrating (l,m) = ", l, m
        WFint[l][m] = GethFromPsi4(WF[l][m], f0*m*0.5)
        if align_time_to_amax: WFint[l][m].x -= tmerger
        elif align_time_to_psi4:
          WFint[l][m].x = (WFint[l][m].x - WFint[l][m].x[0]) + WF[l][m].x[0]
        if m < 0: mstr = 'm%02d' % abs(m)
        else: mstr = 'p%02d' % abs(m)
        fnam = ("%s/h_from_Psi4_scri.L%02dM" % (outdir,l))+ mstr +".dat"
        fout = open(fnam, 'w')
        fout.write("# [1] = t/M\n")
    ...
    m = 0
    for l in range(2, lmax+1):
      if not m0_time_domain: continue
      if self.verbose: print "Integrating (l,m) = ", l, m
      if 'ASCII' in datatype:
        if fstring is not None: fnam = initial_dir+'/'+fstring % (l,m)
        else:
          fnam = ("%s/Psi4_scri.L%02dMp%02d" % (initial_dir,l,m)) + "_uform.asc"
          if self.verbose: print "reading %s" % fnam
      ...
      narr = np.array([simps(parr[:i], x=tarr[:i], dx=tarr[1]-tarr[0]) for i in range(1, len(parr))])
      narr = narr - narr[len(tarr)/2]
      # Get strain by second time-integration
      harr = np.array([simps(narr[:i], x=tarr[:i], dx=tarr[1]-tarr[0]) for i in range(1, len(narr))])
    ...
    if 'HDF' in outputtype:
      if self.verbose: print "Writing Hlm to disk"
...
using the FFI method from http://arxiv.org/abs/1006.1632
  ...
  HDF : inputdir = ., outdir = .
    ...
    for l in range(2,lmax+1):
      for m in range(-l, l+1):
        ...
        fnam = ("%s/h_from_News_scri.L%02dM" % (outdir,l))+ mstr +".dat"
        fout = open(fnam, 'w')
        fout.write("# [1] = t/M\n")
        fout.write("# [2] = Re{rhOverM(%d,%d)}\n" % (l, m))
        fout.write("# [3] = Im{rhOverM(%d,%d)}\n" % (l, m))
    ...
    for l in range(2, lmax+1):
      if not m0_time_domain: continue
      if self.verbose: print "Integrating (l,m) = ", l, 0
      data = datafin[ccegrp]['Y_l%d_m0.dat' % l].value
      tarr = data[:,0]
      tarr -= (tarr[0] - WFint[2][2].x[0])
  ...
  def write_to_hdf5(self, prefix='h_from_Psi4_scri', postfix='.dat', \
    ...
    if grpname in fin.keys() and not replace:
      fin.close()
      os.chdir(pwd)
      return
    elif grpname in fin.keys() and replace: 
    ...
    for l in range(2, lmax + 1):
      for m in range( -l, l + 1):
        ...
        else: fname = self.datafile.replace('h5','joined') + '/' + prefix
        fname = fname + (".L%02dM" % l) + mstr + postfix
        #
        datasetname = 'Y_l%d_m%d.dat' % (l,m)
        fin[grpname].create_dataset(datasetname, data=np.loadtxt(fname))
  ...
  def tar_output(self, remove=True):
    ...
    if nfiles == 0:
      ...
      print >>sys.stderr, "only 1 file found. Its the tar file"
      return
    #
    # To have gotten this far means that Output directories exist
    # Before tarring, make sure that any old tar files are renamed (CRITICAL)
    if os.path.exists(self.prefix+'.tar.gz'):
      cmd.getoutput('mv %s %s' % (self.prefix+'.tar.gz', self.prefix+'.tar.gz.old'))
    to_run = 'tar -cvzf %s.tar.gz `/bin/ls | grep %s`' % \
                                (self.prefix, self.prefix)
    if remove: to_run = to_run + ' --remove-files'
    print >>sys.stderr, "Running %s\n" % to_run
    print >>sys.stdout, cmd.getoutput(to_run)
    os.chdir(pwd)
    return
    #}}}
  #
  def untar_output(self, to_be_extracted=None):
    #{{{
    if to_be_extracted == None: to_be_extracted = self.prefix + '.joined'
    pwd = cmd.getoutput('pwd')
    os.chdir(self.outdir)
    # Check fi the output directory already exists
    if os.path.exists(self.prefix + '.joined'):
      if self.verbose: print "Output joined directory exists. Not Untarring"
      os.chdir(pwd)
      return
    to_run = 'tar -xvzf %s.tar.gz %s' % (self.prefix, to_be_extracted)
    if self.verbose: print >> sys.stderr, "Running %s\n" % to_run
    cmdout = cmd.getoutput(to_run)
    if self.verbose: print >> sys.stdout, cmdout
    #
    os.chdir(pwd)
    return
    #}}}
...
class nr_run_cce():
  ...
  def __init__(self, datadir=None, subdatadir='CceData', outdir=None, pittnull='/home/p/pfeiffer/prayush/src/cactus_cce/Cactus/exe/cactus_pittnull', ccemodeldir='/home/p/pfeiffer/prayush/scratch/projects/CCE_modeldir/', cceparfile='highResCce700.par', increasemaxtimelevels=False, sample_rate=16384, time_length=16, verbose=True):
    ...
    if not os.path.exists(str(datadir)) and os.path.exists(str(outdir)):
      ...
      raise IOError("Must specify either the data or the output directory")
    #
  ...
  def get_nrprefix(self):
    #{{{
    self.nrprefix = None
    if self.datadir is not None: tmp1 = self.datadir.strip('/')
    elif self.outdir is not None: tmp1 = self.outdir.strip('/')
    prefix = tmp1.split('/')[-1]
  ...
  def tar_runs( self, ld, ccefiles, remove=False ):
    #{{{
    if type(ccefiles) != list and type(ccefiles) == str: ccefiles=[ccefiles]
    elif type(ccefiles) != list and type(ccefiles) != str:
      raise RuntimeError("Please pass a single or list of Ccefiles")
    else:
      for cc in ccefiles: self.cce_runs[ld][cc].tar_output(remove=remove)
    return
    #}}}
  ...
  def setup_runs( self, ld, ccefiles, submit=False ):
    ...
    if type(ccefiles) != list and type(ccefiles) == str: ccefiles=[ccefiles]
      ...
      for cc in ccefiles: self.cce_runs[ld][cc].setup_run(submit=submit)
    return
    #}}}
  ...
  def submit_runs( self, ld, ccefiles ):
    ...
    if type(ccefiles) != list and type(ccefiles) == str: ccefiles=[ccefiles]
      ...
      for cc in ccefiles: self.cce_runs[ld][cc].submit_run()
    return
    #}}}
  ...
  def combine_output_for_runs( self, ld, ccefiles, redo=False ):
    ...
    if type(ccefiles) != list and type(ccefiles) == str: ccefiles=[ccefiles]
      ...
      for cc in ccefiles: self.cce_runs[ld][cc].combine_output(redo=redo)
    return
    #}}}
  ...
  def get_psd( self, low_frequency_cutoff=12. ):
    ...
    try:
      if len(self.psd) != N/2+1 or abs(self.psd.delta_f - 1./time_length) > 1.e-12:
        raise RuntimeError("self.psd not in agreement with specified N / df")
    except:
      self.psd = from_txt(\
  ...
  def read_waveforms_from_hdf5_files(self, \
    ...
    for ld in self.levs:
      print >>sys.stdout, "Reading from ", self.outdir+'/'+ld+'/' + wavefilename
      sys.stdout.flush()
      self.wavefiles[ld] = h5py.File(self.outdir+'/'+ld+'/' + wavefilename, 'r')
      self.hwaveforms[ld] = {}
      for kk in self.wavefiles[ld].keys():
  ...
  def read_extrapolated_waveforms_from_hdf5_files(self, dirname=None, \
    ...
    for ld in self.levs:
      self.extrap_wavefiles[ld] = h5py.File(dirname+'/'+ld+'/'+wavefilename,'r')
      self.extrap_hwaveforms[ld] = {}
      for kk in self.extrap_wavefiles[ld].keys():
    ...
    for ld in self.levs:
      ...
      for i1 in range(len(ccefiles)):
        ...
        for i2 in range(i1, len(ccefiles)): # Include self overlaps
          ccef2 = ccefiles[i2]
          if ccef1 not in self.hwaveforms[ld].keys() or \
                ccef2 not in self.hwaveforms[ld].keys():
            print "%s and %s waveforms not found in %s" % (ccef1, ccef2, ld)
            continue
          # Create a group in output file for this ccefile
          if ld+'.dir' not in fout.keys(): fout.create_group(ld + '.dir')
          # Compute matches
          if self.verbose:
            print >>sys.stderr, "Overlaps for %s Between %s and %s" % (ld, ccef1, ccef2)
          overlaps = overlaps_vs_totalmass(self.hwaveforms[ld][ccef1],\
                      self.hwaveforms[ld][ccef2], psd=self.psd, \
                      m_upper=m_upper, m_delta=m_delta)
          # Add matches and masses as a dataset to the group
          dsetname = ccef1 + '_' + ccef2 + '.dat'
          if self.verbose: print "Creating dataset ", dsetname
          if self.verbose:
            print "keys: ", fout.keys(), ld, fout[ld+'.dir'].keys()
          #if self.verbose: print "overlaps", overlaps, np.shape(overlaps)
  ...
  def plot_mismatches_from_hdf5(self, matchfilename='matches/OverlapsLevs.h5', \
    ...
    colors = ["blue","red","green","magenta","cyan","gold","darkorange"] # blending options
    l_blend = ["A","B","C","D","E"]
    l_compare = []
    plot_lines = []
    #
    for kk in range(len(constt_keys)):
      ...
      if prefix is None: plotname = self.outdir + '/' + outdir + '/' + const_key + '.png'
      else: plotname = self.outdir + '/' + outdir + '/' + prefix + const_key + '.png'
      ...
      # 
      plot_lines = [ [],[],[],[],[],[] ]
      print mismatch[mismatch.keys()[0]], mismatch.keys()
      l_compare = []
      for i in range(len(mismatch[mismatch.keys()[0]])):
        for n in range(len(mismatch.keys())):
          print "i = %d/%d, n = %d/%d" % (i,len(mismatch[mismatch.keys()[0]]), n, len(mismatch.keys()))
          ...
          pl, = plt.plot(mass[var_key],mismatch[var_key][i],linestyle=line[n],marker=markers[n],color=colors[i])
      ...
      if 'Lev' in var_keys[const_key][0]:
        legend_2 = plt.legend(zip(*plot_lines)[0], l_blend, ncol=2, loc="lower right")
        plt.title("Mismatch with blended waveforms, at Fixed "+const_key+"\n"+self.get_nrprefix())
  ...
  def calculate_mismatch_with_extrapolated_hdf5(self, \
    ...
    try: tmp_len = len(self.hwaveforms)
    except: self.read_waveforms_from_hdf5_files(wavefilename=wavefilename)
    # Get the Extrapolated waveforms for different Extrapolation ORders
    print "Reading Extrapolated waveforms"
    try: tmp_len = len(self.extrap_hwaveforms)
    except:
      if extrap_runname is None:
        extrap_runname = extrapolated_outdir_from_cce_outdir(self.outdir)
    ...
    Levs = self.levs
    #CceRindices = [-1]  
    CceRindices = np.arange(len(self.wavefiles[Levs[0]].keys()))
    #ExtrapOrders = ['Extrapolated_N2.dir','Extrapolated_N3.dir','OutermostExtraction.dir'] 
    ExtrapOrders = self.extrap_hwaveforms[Levs[0]].keys()
    all_ccefiles = self.wavefiles[self.levs[0]].keys()
    all_ccefiles.sort()
    ccefiles = [all_ccefiles[idx] for idx in CceRindices]
    #
    print Levs, "\n", ccefiles, "\n", CceRindices, "\n", ExtrapOrders
    # At each Lev, compare waveforms at all cce radii with extrapolation orders
    for lev in Levs:
      print "At ", lev
      if lev not in self.hwaveforms.keys() or lev not in self.extrap_hwaveforms.keys():
        print "No waveforms at %s in either the Cce or Extrapolated set" % lev
        continue
      if lev+'.dir' not in fout.keys(): fout.create_group(lev + '.dir')
      for ccef in ccefiles:
        print "For ", ccef
        if ccef not in self.hwaveforms[lev].keys():
          print ccef, " waveform not found for %s" % lev
        hp0 = self.hwaveforms[lev][ccef]
        for extrap_order in ExtrapOrders:
          # Safegaurd for N-1 orders. New convention is OutermostExtrapolation
          if 'N-1' in extrap_order:
            print "Skipping ", extrap_order, " at ", lev
            continue
          #
          print "At ", extrap_order
          if extrap_order not in self.extrap_hwaveforms[lev].keys():
            print "%s waveform not found at %s" % (extrap_order, lev)
