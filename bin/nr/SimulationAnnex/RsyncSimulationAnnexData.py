...
parser.add_option(
    "-s", "--start", help="Index to start from [default 0]", type=int, default=0
)
parser.add_option(
    "-e", "--end", help="Index to start from [default 0]", type=int, default=-1
)

parser.add_option(
    "--overwrite", action="store_true", help="Overwrite data", default=False
)
...
for row_id, row in enumerate(input_table):
    ...
    try:
        ...
        remote_wave_file = row.numrel_data.replace("/", r"\/").replace(":", r"\:")
        ...
        if options.overwrite or not os.path.exists(
            ...
            if (
                os.path.exists(os.path.join(local_wave_dir, "metadata.txt"))
                and options.verbose
            ):
            ...
            if options.verbose:
                print(
                    "Metadata for {} already exists. Skipping..".format(local_wave_dir)
                )
        if options.overwrite or not os.path.exists(
            ...
            if options.verbose:
                print(
                    "Data {} for {} already exists. Skipping..".format(
