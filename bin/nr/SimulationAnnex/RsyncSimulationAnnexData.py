...
parser.add_option("-s", "--start",
                  ...
                  type=int, default=0 )
parser.add_option("-e", "--end",  help="Index to start from [default 0]",
                  type=int, default=-1 )
...
for row_id, row in enumerate(input_table):
    if row_id < options.start: continue
    if options.end > 0 and row_id > options.end: continue
    try:
        ipwd = cmd.getoutput('pwd -P')
        _ = cmd.getoutput('mkdir -p {}'.format(row.waveform))
        local_wave_dir   = row.waveform.replace('/','\/').replace(':','\:')
        remote_wave_file = row.numrel_data.replace('/','\/').replace(':','\:')
        remote_wave_filename = os.path.split(remote_wave_file)[-1]
        remote_metadata_file = remote_wave_file.replace(remote_wave_filename, 'metadata.txt')
        if options.overwrite or not os.path.exists(os.path.join(local_wave_dir, 'metadata.txt')):
            cmd_to_run = 'cd {} && rsync --progress -avL -e ssh rrsync@black-holes.org:{} .'.format(\
                                  local_wave_dir, remote_metadata_file)
            if os.path.exists(os.path.join(local_wave_dir, 'metadata.txt')) and options.verbose:
                print "Metadata being OVERWRITTEN."
            out = cmd.getoutput(cmd_to_run)
        else:
            if options.verbose:
                print "Metadata for {} already exists. Skipping..".format(local_wave_dir)
        if options.overwrite or not os.path.exists(os.path.join(local_wave_dir, remote_wave_filename)):
            cmd_to_run = 'cd {} && rsync --progress -avL -e ssh rrsync@black-holes.org:{} .'.format(\
            ...
            if options.verbose:
                print "Data {} for {} already exists. Skipping..".format(remote_wave_filename,\
                                                                         local_wave_dir)
