...
def get_waveform_location(
    ...
    if options.use_hdf:
        ...
        h22file = (
            workdir + "/" + dirname + "/" + subdir + "/highResCce.joined/" + filename
        )
    if os.path.exists(h22file) and os.path.getsize(h22file) > 0:
        ...
        print("Waveform SYMLINK found for %s at %s" % (tag, h22file), file=sys.stdout)
        return h22file
    else:
...
parser.add_option("--f-lower", type=float, help="Low f cutoff", default=15.0)
...
parser.add_option("--mass-sampling-step", type=float, help="dM", default=1.0)
...
if options.input_catalog is not None:
    ...
    for point in input_table:
        if options.verbose:
            print(
                " .. Preparing injections for %s" % point.numrel_data, file=sys.stdout
            )
        if not os.path.exists(point.numrel_data):
            print(
                "waveform file %s for %s does NOT exist! "
        ...
        idx = 0
        for mass in np.arange(
            mLowMassCutoff, options.upper_mass_threshold, options.mass_sampling_step
        ):
