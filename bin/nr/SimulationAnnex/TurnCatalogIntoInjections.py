...
if options.input_catalog is not None:
    ...
    for point in input_table:
        ...
        while True:
            ...
            if options.verbose:
                print(
                    " .. .. low-M cutoff from catalog = %f Msun" %
                    mLowMassCutoff, file=sys.stdout)
            if m_lower > mLowMassCutoff:
                mLowMassCutoff = m_lower
            if options.verbose:
                print(
                    " .. .. low-M cutoff after 1100M window = %f Msun" %
                    mLowMassCutoff, file=sys.stdout)
            break
        ...
        for mass in np.arange(mLowMassCutoff, options.upper_mass_threshold,
            ...
            if idx == 0:
                if options.verbose:
                    print(
                        " .. scaling for lowest M to check ..",
                        file=sys.stdout)
                try:
                    nrwav.rescale_to_totalmass(mass)
                except IOError:
                    print(
                        " ... FAILED to GENERATE, moving on.. ",
                        file=sys.stdout)
                    sys.stdout.flush()
                    continue
                sys.stdout.flush()
