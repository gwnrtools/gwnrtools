...
import UseNRinDA_NRWave as UseNRinDA
...
def get_waveform_location( p, cce_filename_ascii='h_from_Psi4_scri.L02Mp02.dat',\
  ...
  if wavetype == 'cce': filename = cce_filename
...
parser.add_option("--nr-input-dir", metavar='DIR', help='DEFUNCT: Main dir with nr sim',default='/home/p/pfeiffer/prayush/scratch/projects/CCE/ChuAlignedSpinning/')

parser.add_option("--f-lower", type=float, help="Low f cutoff", default=15. )
parser.add_option("--sample-rate", type=float, help="WF sample rate", default=4096 )
parser.add_option("--upper-mass-threshold", type=float, help="Upper limit for total mass sampling", default=150. )
...
parser.add_option("-F", "--force-file-exists", action="store_true", help="Only add injections if the NR data file exists", default=False )
...
parser.add_option("--zero-transverse-spins", action="store_true", help="if transverse spins are smaller than spin-threshold, set them to 0", default=True )
...
if options.input_catalog is not None:
  ...
  for point in input_table:
    if options.verbose:
      print >>sys.stdout, " .. Preparing injections for %s" % point.numrel_data
    ...
    while True:
      ...
      nrwav = UseNRinDA.nr_wave(filename=point.numrel_data, modeLmax=2, \
                    sample_rate=options.sample_rate, \
                    time_length=estimated_length_pow2, \
                    totalmass=total_mass, inclination=0, phi=0, \
                    distance=1e6, ex_order=3)
      m_lower = nrwav.get_lowest_binary_mass( 1100, options.f_lower)
      if options.verbose:
        print >>sys.stdout, \
          " .. .. low-M cutoff from catalog = %f Msun" % mLowMassCutoff
      if m_lower > mLowMassCutoff: mLowMassCutoff = m_lower
      if options.verbose:
        print >>sys.stdout, \
          " .. .. low-M cutoff after 1100M window = %f Msun"% mLowMassCutoff
      break
    ...
    for mass in np.arange( mLowMassCutoff, options.upper_mass_threshold, \
      ...
      if idx == 0:
        if options.verbose:
          print >>sys.stdout, " .. scaling for lowest M to check .."
        try: nrwav.rescale_to_totalmass(mass)
        except IOError:
          print >>sys.stdout, " ... FAILED to GENERATE, moving on.. "
          sys.stdout.flush()
          continue
        sys.stdout.flush()
      #
      ...
      if options.zero_transverse_spins:
        ...
        if abs(npoint.spin2y) <= options.transverse_spin_threshold:
          npoint.spin2y = 0
      else: print "NOT zero-ing small x,y spins"
