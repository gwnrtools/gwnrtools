...
if options.input_catalog is not None:
    ...
    for point in input_table:
        if options.verbose:
            print(" .. Preparing injections for %s" % point.numrel_data,
                  file=sys.stdout)
        ...
        while True:
            ...
            estimated_length_pow2 = nextpow2(MAX_NR_LENGTH * total_mass *
                                             lal.MTSUN_SI)
            nrwav = UseNRinDA.nr_wave(filename=point.numrel_data,
                                      modeLmax=2,
            ...
            if options.verbose:
                print(" .. .. low-M cutoff from catalog = %f Msun" %
                      mLowMassCutoff,
                      file=sys.stdout)
            if m_lower > mLowMassCutoff:
                mLowMassCutoff = m_lower
            if options.verbose:
                print(" .. .. low-M cutoff after 1100M window = %f Msun" %
                      mLowMassCutoff,
                      file=sys.stdout)
            break
