...
parser.add_argument(
    "--new-proposal-file-name", dest="new_prop_file_name", help="The new points file"
)
parser.add_argument(
...
new_inspiral_table = lsctables.New(
    lsctables.SimInspiralTable, columns=prop_table.columnnames
)
...
for mfile_name in mfile_names:
    mvals_for_each_bank_point, mvals_for_each_test_point = parse_match_file(
        mfile_name, mvals_for_each_bank_point, mvals_for_each_test_point
    )
## 3)
...
if options.verbose:
    ...
    logging.info(
        "Total {} test points eliminated, {} left.".format(
