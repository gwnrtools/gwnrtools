...
# ~ type=int)
parser.add_argument("-t", "--tmplt-bank", metavar="file", help="template bank to split")

parser.add_argument(
    "-n", "--num", metavar="SAMPLES", help="number of output banks", type=int
)
parser.add_argument(
    "--mchirp-min",
    metavar="MC_MIN",
...
parser.add_argument("-e", "--named", help="Starting string in the names of final XMLs")
...
n_should_be = int(
    np.ceil(np.log(mc_max / mc_min) / np.log(1.0 + options.mchirp_window))
)
...
mc_bins_edges = np.array(
    [mc_min * (1.0 + options.mchirp_window) ** i for i in range(options.num + 1)]
)
...
for i in range(len(mc_bins_edges) - 1):
    ...
    if options.verbose:
        logging.info(
            "\t {} templates in sub-bank {}.".format(len(new_inspiral_table), i)
        )
