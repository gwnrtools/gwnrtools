...
def align_waveforms_amplitude_peak(hplus1, hcross1, hplus2, hcross2):
    ...
    x0 = np.float64(
        np.where(amp1.data == max(amp1.data))[0][0] * amp1.delta_t + amp1._epoch
    )
    tmp = minimize_scalar(
        amp1I,
        x0,
    ...
    x0 = np.float64(
        np.where(amp2.data == max(amp2.data))[0][0] * amp2.delta_t + amp2._epoch
    )
    tmp = minimize_scalar(
        amp2I,
        x0,
    ...
    hp1, hc1 = shift_waveform_phase_time(hp1, hc1, 0, -0 - 1.0 * ph1)
    hp2, hc2 = shift_waveform_phase_time(hp2, hc2, t_shift, -0 - 1.0 * ph2)
    #
...
def align_waveforms_at_frequency(hplus1, hcross1, hplus2, hcross2, falign):
    ...
    for idx in range(id_start, len(freq1)):
        if freq1[idx] > 2 * falign and freq1[idx + 1] > 2 * falign:
            break
    tmp = minimize_scalar(f1I, id_start, method="bounded", bounds=(id_start, idx))
    ...
    for idx in range(id_start, len(freq2)):
        if freq2[idx] > 2 * falign and freq2[idx + 1] > 2 * falign:
            break
    tmp = minimize_scalar(f2I, id_start, method="bounded", bounds=(id_start, idx))
    ...
    # Enforced that phase at merger be 0 for BOTH
    hp1, hc1 = shift_waveform_phase_time(hp1, hc1, 0, -0 - 1.0 * ph1)
    hp2, hc2 = shift_waveform_phase_time(hp2, hc2, 1 * t_shift, -0 - 1.0 * ph2)
    #
    return hp1, hc1, hp2, hc2
...
def align_waveforms_optimally(
    ...
    h_cross2 = TimeSeries(
        hcross2, epoch=hcross2._epoch, delta_t=hcross2.delta_t, dtype=hcross2.dtype
    )
    #
    if psd is None:
    ...
    if f_high is None:
        f_high = 1.0 / h_plus1.delta_t / 2.0
    #
    ...
    snr, corr, snr_norm = matched_filter_core(htilde, stilde, psd, f_low, f_high, None)
    ...
    hp2, hc2 = shift_waveform_phase_time(
        h_plus2, h_cross2, tsign * t_shift, phsign * ph_shift
    )
    return h_plus1, h_cross1, hp2, hc2


def make_inspiral_merger_plot(
    hp1,
    hc1,
    hp2,
    hc2,
    ...
    ax0.plot(
        np.arange(len(hp1)) * tscale, hp1, np.arange(len(hp2)) * tscale, hp2, lw=0.5
    )
    ...
    xmin, xmax = (
        min(max_id1, max_id2) * lp * tscale,
        min(max_id1, max_id2) * rp * tscale,
    )
...
def make_inspiral_merger_plot_3(
    hp1,
    hc1,
    hp2,
    hc2,
...
nrwav = UseNRinDA.nr_waveform(
    filename="/home/prayush/research/cita-papers/SEOBtesting/Paper1/plots/InvestigateSimulations/d11.5-q8.5-sA_0_0_0_sB_0_0_0/rhOverM_CcePITT_Asymptotic_GeometricUnits.h5",
    time_length=8,
)
...
nrwav2 = UseNRinDA.nr_waveform(
    filename="/home/prayush/research/cita-papers/SEOBtesting/Paper1/plots/InvestigateSimulations/d11.5-q8.5-sA_0_0_0_sB_0_0_0/rhOverM_Asymptotic_GeometricUnits.h5",
    time_length=8,
)
...
# distance=(nrm1+nrm2)*lal.MRSUN_SI/(1.e6*lal.PC_SI), \
# delta_t=nrwav.dt, f_lower=f_low-1.)
# hp0 = TimeSeries(np.zeros(nrwav.n), delta_t=hp0t.delta_t, dtype=hp0t.dtype, epoch=0)
# hc0 = TimeSeries(np.zeros(nrwav.n), delta_t=hc0t.delta_t, dtype=hc0t.dtype, epoch=0)
...
# align at 20 Hz
hnrhp, hnrhc, hhp0, hhc0 = align_waveforms_at_frequency(nrhp, nrhc, hp0, hc0, 20)
...
hp1t, hc1t = get_td_waveform(
    approximant="SEOBNRv2",
    mass1=nrm1,
    mass2=nrm2,
    spin1z=nrs1,
    spin2z=nrs2,
    distance=(nrm1 + nrm2) * lal.MRSUN_SI / (1.0e6 * lal.PC_SI),
    delta_t=nrwav.dt,
    f_lower=f_low - 1.0,
)
hp1 = TimeSeries(np.zeros(nrwav.n), delta_t=hp1t.delta_t, dtype=hp1t.dtype, epoch=0)
hc1 = TimeSeries(np.zeros(nrwav.n), delta_t=hc1t.delta_t, dtype=hc1t.dtype, epoch=0)
