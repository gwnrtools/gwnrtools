...
psd = from_txt(
    "/home/prayush/research/advLIGO_PSDs/ZERO_DET_high_P.dat", N, 1.0 / T, f_low
)
...
if True:
    T = 1.0 / 30.0
    tvec = np.arange(0, 100 * T, 1.0 / sample_rate)
    hp1 = TimeSeries(np.cos(tvec * 2 * np.pi / T), delta_t=1.0 / sample_rate)
    hc1 = TimeSeries(-1 * np.sin(tvec * 2 * np.pi / T), delta_t=1.0 / sample_rate)
...
if True:
    T = 1.0 / 30.0
    tvec = np.arange(0, 100 * T, 1.0 / sample_rate)
    hp2 = TimeSeries(np.cos(tvec * 2 * np.pi / T), delta_t=1.0 / sample_rate)
    hc2 = TimeSeries(-1 * np.sin(tvec * 2 * np.pi / T), delta_t=1.0 / sample_rate)
...
for i in range(1400, idx_length, int(0.0005 / hp1.delta_t)):
    ...
    ax.plot(
        matchCplx.sample_times,
        np.abs(matchCplx),
        "k",
        matchCplx.sample_times[i],
        np.abs(mCplx),
        "ro",
    )
    ax.grid()
    ax.set_xlim(
        0 + np.float64(matchCplx._epoch),
