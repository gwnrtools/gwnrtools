...
parser.add_option(
    "-m", "--output-mapfile", help="output file to store template-to-NR-data mappings"
)
parser.add_option(
    "-a",
    "--need-all-mappings",
...
if options.input_bank is not None:
    indoc = ligolw_utils.load_filename(
        options.input_bank, contenthandler=LIGOLWContentHandler, verbose=options.verbose
    )
    #
    try:
...
for idx, tmplt in enumerate(input_bank):
    ...
    if is_map:
        ...
        if options.verbose and idx % NOUTPUT == 0:
            ...
            print(" .. .. .. file exists: ", os.path.exists(mapstring), file=sys.stderr)
        #
