...
class test:
    ...
    def m1_m2_to_tau0_tau3(self, m1, m2, f_min):
        ...
        et = m1 * m2 / mt**2
        tau3 = 1.0 / (
            8.0 * (pi * pi * f_min**5.0) ** (1.0 / 3.0) * mt ** (2.0 / 3.0) * et
        )
...
parser.add_option(
    "--min-sz", help="lower spinZ bound", dest="min_sz", type=float, default=-0.9
)
parser.add_option(
    "--max-sz", help="upper spinZ bound", dest="max_sz", type=float, default=0.9
)
parser.add_option(
    "--inclination",
...
for i in np.arange(npoints):
    ...
    if options.eta is not None:
        ...
        mtot_min = (
            ...
            * (
                1.0
                + 15.1506 * smplpt.eta
                - 96.7042 * smplpt.eta**2
                + 305.441 * smplpt.eta**3
                - 342.732 * smplpt.eta**4
            )
            * (
                1.0
                + 0.304976 * chi
    ...
    while not accept_point_alignedspin(
        smplpt.mchirp / smplpt.eta**0.6, smplpt.eta, chi
    ):
        ...
        if (smplpt.spin1x or smplpt.spin1y or smplpt.spin1z) and not (
            options.spin1x or options.spin1y or options.spin1z
        ):
            s1mag = np.sqrt(
        ...
        if (smplpt.spin2x or smplpt.spin2y or smplpt.spin2z) and not (
            options.spin2x is not None
            or options.spin2y is not None
