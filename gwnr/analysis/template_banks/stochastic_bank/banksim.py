...
from pycbc.scheme import CUDAScheme, DefaultScheme
...
def generate_fplus_fcross(latitude, longitude, polarization):
    f_plus = -(1.0 / 2.0) * (1.0 + cos(latitude) * cos(latitude)) * cos(
        2.0 * longitude
    ) * cos(2.0 * polarization) - cos(latitude) * sin(2.0 * longitude) * sin(
        2.0 * polarization
    )
    f_cross = (1.0 / 2.0) * (1.0 + cos(latitude) * cos(latitude)) * cos(
        2.0 * longitude
    ) * sin(2.0 * polarization) - cos(latitude) * sin(2.0 * longitude) * cos(
        2.0 * polarization
    )
...
def make_padded_frequency_series(vec, filter_N=None):
    ...
    if isinstance(vec, FrequencySeries):
        vectilde = FrequencySeries(
            zeros(n, dtype=complex_same_precision_as(vec)), delta_f=1.0, copy=False
        )
    ...
    if isinstance(vec, TimeSeries):
        vec_pad = TimeSeries(
            zeros(N), delta_t=vec.delta_t, dtype=real_same_precision_as(vec)
        )
        vec_pad[0 : len(vec)] = vec
        delta_f = 1.0 / (vec.delta_t * N)
        vectilde = FrequencySeries(
            zeros(n), delta_f=1.0, dtype=complex_same_precision_as(vec)
        )
        fft(vec_pad, vectilde)

    vectilde = FrequencySeries(
        vectilde * DYN_RANGE_FAC, delta_f=delta_f, dtype=complex64
    )
    return vectilde
...
def get_waveform(
    ...
    if approximant in td_approximants():
        ...
        if hasattr(template_params, "spin1z") and hasattr(template_params, "spin2z"):
        ...
        if True:
            ...
            print(
                "m1 = %f, m2 = %f" % (template_params.mass1, template_params.mass2),
                file=sys.stderr,
            )
        hvec = get_fd_waveform(
            template_params,
            approximant=approximant,
            phase_order=phase_order,
            delta_f=delta_f,
        ...
        if True:
            print(
                "filter_N = %d in get waveform" % filter_N,
...
parser = OptionParser()
parser.add_option(
    "--match-file", dest="out_file", help="file to output match results", metavar="FILE"
)
...
parser.add_option(
    "--psd",
    dest="psd",
    help="Analytic PSD model: " + str(pycbc.psd.get_list()),
    choices=pycbc.psd.get_list(),
)
...
parser.add_option(
    "--template-file",
    dest="bank_file",
    help="SimInspiral or SnglInspiral XML file containing the template parameters.",
    metavar="FILE",
)
parser.add_option(
...
parser.add_option(
    "--template-phase-order", help="PN order to use for the phase", default=-1, type=int
)
parser.add_option(
    "--template-amplitude-order",
    help="PN order to use for the amplitude",
    default=-1,
    type=int,
)
parser.add_option(
    "--template-start-frequency", help="Starting frequency for injections", type=float
)
parser.add_option(
    "--template-sample-rate", help="Starting frequency for injections", type=float
)
...
parser.add_option(
    "--signal-approximant", help="Signal Approximant Name: " + str(aprs), choices=aprs
)
parser.add_option(
    "--signal-phase-order", help="PN order to use for the phase", default=-1, type=int
)
parser.add_option(
    "--signal-amplitude-order",
    help="PN order to use for the amplitude",
    default=-1,
    type=int,
)
parser.add_option(
    "--signal-start-frequency", help="Starting frequency for templates", type=float
)
parser.add_option(
    "--signal-sample-rate", help="Starting frequency for templates", type=float
)
...
parser.add_option(
    ...
    help="Length of signal for filtering, shoud be longer than all waveforms and include some padding",
    type=int,
)
...
if options.asd_file:
    psd = pycbc.psd.from_asd_txt(
        options.asd_file, filter_n, filter_delta_f, options.filter_low_frequency_cutoff
    )
elif options.psd:
    psd = pycbc.psd.from_string(
        options.psd, filter_n, filter_delta_f, options.filter_low_frequency_cutoff
    )
...
for signal_params in signal_table:
    ...
    s_norm = sigmasq(
        stilde, psd=psd, low_frequency_cutoff=options.filter_low_frequency_cutoff
    )
    stilde /= psd
...
for stilde, s_norm, matches, sim_template in signals:
    match_str = "%5.5f \n" % (max(matches))
    match_str2 = (
        "  " + options.bank_file + " " + str(matches.index(max(matches))) + "\n"
    )
