...
def within_region_alignedspin(mt, et, chi):
    feta = (
        et**-0.6
        * 12.9241
        * (1.0 + 15.1506 * et - 96.7042 * et**2 + 305.441 * et**3 - 342.732 * et**4)
        * (
            1.0
            + 0.304976 * chi
            + 0.140933 * chi**2
...
if (
    options.mtotal_cut is None
    and options.mchirp_cut is None
    and options.eta_cut is None
    and options.mass_cut is None
    and options.extract_eta is None
    and options.extract_q is None
    and not options.strict_region
    and options.upper_spinz_cut is None
    and options.lower_spinz_cut is None
):
...
for bank_point in in_bank_table:
    ...
    if options.strict_region:
        ...
        if options.extract_q is not None:
            options.extract_eta = options.extract_q / (1.0 + options.extract_q) ** 2
        ...
        if options.lower_spinz_cut is None:
            options.lower_spinz_cut = -1.0
        if options.upper_spinz_cut is None:
            options.upper_spinz_cut = 1.0
        if (
            bank_point.spin1z >= options.lower_spinz_cut
            and bank_point.spin1z <= options.upper_spinz_cut
            and bank_point.spin2z >= options.lower_spinz_cut
            and bank_point.spin2z <= options.upper_spinz_cut
        ):
            out_subbank_table.append(bank_point)
        else:
