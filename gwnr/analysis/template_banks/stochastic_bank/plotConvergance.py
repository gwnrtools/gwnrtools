...
for i in range(max(idx)):
    if i != max(idx):
        ...
        ncvg.append(i + 1)
        print(
            ("Acceptance while making bank %d is %f" % (ncvg[i], 100.0 * cvg[i])) + " %"
        )
...
for idx in range(NUMBANKSMADE):
    ...
    pi += 1
    pb.plot(m1, m2, "x")
    pb.xlim([min(m1) - 0.5, max(m1) + 0.5])
    pb.ylim([min(m2) - 0.5, max(m2) + 0.5])
    pb.grid()
    pb.title("Bank %d" % idx)
    ...
    pi += 1
    pb.plot(mc, et, "x")
    pb.xlim([min(mc) - 0.5, max(mc) + 0.5])
    pb.ylim([min(et) - 0.01, max(et) + 0.01])
    pb.grid()
    pb.title("Bank %d" % idx)
    ...
    pi += 1
    pb.plot(mt, s1z, "x")
    pb.xlim([0.0, max(mt) + 0.5])
    pb.ylim([min(s1z) - 0.05, max(s1z) + 0.05])
    pb.grid()
    pb.title("Bank %d" % idx)
