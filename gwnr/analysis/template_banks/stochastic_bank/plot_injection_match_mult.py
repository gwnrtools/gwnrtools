...
def eta_to_q(eta):
    q = (1.0 + sqrt(1.0 - 4.0 * eta)) / (2.0 * eta) - 1.0
    return q
...
for n in range(options.num):
    ...
    for injection in sim_table:
        ...
        t0, t3 = qm.m1_m2_to_tau0_tau3(
            injection.mass2 * qm.QM_MTSUN_SI, injection.mass1 * qm.QM_MTSUN_SI, 40
        )
        tau0.append(t0)
        tau3.append(t3)
...
sngl_inspiral_table = table.get_table(indoc, lsctables.SimInspiralTable.tableName)

for template in sngl_inspiral_table:
    t0, t3 = qm.m1_m2_to_tau0_tau3(
        template.mass2 * qm.QM_MTSUN_SI, template.mass1 * qm.QM_MTSUN_SI, 40
    )
    ttau0.append(t0)
    ttau3.append(t3)
...
chi1cut = []
chi2cut = []
mtotalthreshold = 140.0

qthreshold = 4.0 / 3.0
etathreshold = 0.0  # qthreshold / (1. + qthreshold)**2.
MMthreshold = 0.97
...
for index in range(len(mass1)):
    if matches[index] < MMthreshold:
        if matches[index] < 0.97:  # and eta[index] >= (10./121.):
...
pylab.hist(
    matches, bins=nbins, normed=True, cumulative=True, log=True, color="blue", alpha=0.6
)
pylab.grid()
pylab.xlim(xmax=1.0)
pylab.xlabel("FF (X)")
...
x_min = min(mass1)
x_max = 100.0  # max(mass1)

y_min = min(mass2cut)
y_max = 100.0  # max(mass2cut)

qline1 = pylab.arange(3.0 * qthreshold, 25.001, 0.001)
...
pylab.scatter(mass1, mass2, c=matches, s=0.5, linewidths=0, vmin=0.98)
...
pltid += 1
pylab.axes([0.125, 0.125, 0.95 - 0.125, 0.95 - 0.2])
pylab.scatter(mass1cut, mass2cut, c=matchescut, s=0.5, vmin=0.9, linewidths=0)
# pylab.hexbin(mass1cut,mass2cut,matchescut,vmin=0.96)
cb = pylab.colorbar()
cb.set_label("FF")
# pylab.scatter(sngl_inspiral_table.get_column('mass1'),sngl_inspiral_table.get_column('mass2'),s=.1,linewidths=0,c='black')
...
pltid += 1
pylab.scatter(tau0, tau3, c=matches, s=0.05, linewidths=0)
cb = pylab.colorbar()
cb.set_label("Effectualness")
pylab.scatter(ttau0, ttau3, s=0.1, linewidths=0, c="black")
...
pltid += 1
pylab.axes([0.125, 0.125, 0.95 - 0.125, 0.95 - 0.2])
pylab.scatter(eta, chi1, c=matches, s=0.5, vmin=0.98, vmax=1.0, linewidths=0)
pylab.ylim([-1.05, 1.05])
pylab.xlim([min(eta) - 0.01, max(eta) + 0.01])
pylab.colorbar()
...
pltid += 1
pylab.axes([0.125, 0.125, 0.95 - 0.125, 0.95 - 0.2])
pylab.scatter(eta, chi2, c=matches, s=0.5, vmin=0.98, vmax=1.0, linewidths=0)
pylab.ylim([-1.05, 1.05])
pylab.xlim([0.0, max(eta) + 0.1])
pylab.colorbar()
...
pltid += 1
pylab.axes([0.125, 0.125, 0.95 - 0.125, 0.95 - 0.2])
pylab.scatter(q, chi1, c=matches, s=0.5, vmin=0.98, vmax=1.0, linewidths=0)
pylab.ylim([-1.05, 1.05])
pylab.xlim([min(q) - 1.0, max(q) + 1])
pylab.colorbar()
...
pltid += 1
pylab.axes([0.125, 0.125, 0.95 - 0.125, 0.95 - 0.2])
pylab.scatter(q, chi2, c=matches, s=0.5, vmin=0.98, vmax=1.0, linewidths=0)
pylab.ylim([-1.05, 1.05])
pylab.xlim([min(q) - 1.0, max(q) + 1])
pylab.colorbar()
...
pylab.scatter(mchirp, eta, c=matches, s=0.5, vmin=0.96, linewidths=0)
...
pltid += 1
pylab.axes([0.125, 0.125, 0.95 - 0.125, 0.95 - 0.2])
pylab.scatter(mchirp, mtotal, c=matches, s=0.5, linewidths=0)
pylab.colorbar()
...
pltid += 1
pylab.axes([0.125, 0.125, 0.95 - 0.125, 0.95 - 0.2])
pylab.scatter(mtotal, eta, c=matches, s=0.5, vmin=0.96, linewidths=0)
pylab.colorbar()
...
pltid += 1
pylab.axes([0.125, 0.125, 0.95 - 0.125, 0.95 - 0.2])
pylab.scatter(mtotal, chi1, c=matches, s=0.5, vmin=0.98, vmax=1.0, linewidths=0)
pylab.ylim([-1.05, 1.05])
pylab.xlim([0.0, max(mtotal) + 1.0])
pylab.colorbar()
...
pltid += 1
pylab.axes([0.125, 0.125, 0.95 - 0.125, 0.95 - 0.2])
pylab.scatter(mtotalcut, chi1cut, c=matchescut, s=0.5, linewidths=0)
pylab.colorbar()
...
pltid += 1
pylab.axes([0.125, 0.125, 0.95 - 0.125, 0.95 - 0.2])
pylab.scatter(mtotal, chi2, c=matches, s=0.5, vmax=1.0, linewidths=0)
pylab.ylim([-1.05, 1.05])
pylab.xlim([0.0, max(mtotal) + 1.0])
pylab.colorbar()
...
pltid += 1
pylab.axes([0.125, 0.125, 0.95 - 0.125, 0.95 - 0.2])
pylab.scatter(mtotalcut, chi2cut, c=matchescut, s=0.5, linewidths=0)
pylab.colorbar()
...
pylab.hold(True)
pylab.plot(tmpltq, tmpltch, "kx", markersize=3.0)
# pylab.hist2d(qcut1, chi1cut1, bins = nbins, range=None, weights=None, cmin=None, cmax=None )
pylab.grid()
pylab.xlim(xmax=1.0)
pylab.ylim([min(chi1cut1) - 0.1, max(chi1cut1) + 0.1])
pylab.xlim([min(qcut1) - 1.0, max(qcut1) + 1.0])
pylab.xlabel("$q$")
pylab.ylabel("$\chi_1$")
...
pylab.grid()
pylab.xlim(xmax=1.0)
pylab.ylim([min(chi1) - 0.1, max(chi1) + 0.1])
pylab.xlim([min(q) - 1.0, max(q) + 1.0])
pylab.xlabel("$q$")
pylab.ylabel("$\chi_1$")
...
colormap = matplotlib.mlab.griddata(
    q, chi1, matches, qmap, chi1map
)  # [(1.-x) for x in matches], qmap, chi1map)
...
pylab.contourf(
    qmap, chi1map, colormap, [0.93, 0.95, 0.965, 0.97, 0.98, 0.99, 1.0]
...
pylab.ylim(y_min - 0.01, y_max + 0.01)
pylab.xlabel("$q$")
pylab.ylabel("$\chi_1$")
pylab.grid()
