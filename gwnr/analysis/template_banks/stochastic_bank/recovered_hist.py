...
while isfile(options.bank_file + str(index) + ".xml"):
    indoc = ligolw_utils.load_filename(
        options.bank_file + str(index) + ".xml", options.verbose
    )
...
for index in range(0, len(matches)):
    ...
    itau0, itau3 = qm.m1_m2_to_tau0_tau3(
        injected[index].mass1, injected[index].mass2, 15
    )
    if hasattr(recovered[index], "mass1"):
        rtau0, rtau3 = qm.m1_m2_to_tau0_tau3(
            recovered[index].mass1, recovered[index].mass2, 15
        )
    else:
...
if not options.max_total_mass:
    pylab.figure(1)
    pylab.plot(
        [min(inj_eta), max(inj_eta)], [min(inj_eta), max(inj_eta)], "-", color="black"
    )
    ...
    n, bins, patches = pylab.hist(
        mchirp_err, bins=100, normed=True, histtype="stepfilled"
    )
