...
parser = OptionParser()
parser.add_option(
    "--iteration-id", help="The index of the iteration", type=int, dest="iid"
)
parser.add_option(
...
parser.add_option(
    ...
    "--mchirp-window",
    metavar="MC_WIN",
    help="Fractional window on mchirp parameter. If waveform parameters differ by more than this window, the overlap is set to 0.",
    default=0.01,
    type=float,
    dest="mchirp_window",
)
...
inc_min = 0.0
inc_max = 0.0  # qm.QM_PI

pol_min = 0.0
pol_max = 0.0  # qm.QM_PI * 2.
...
def get_new_sample_point():
    ...
    p.spin1x = 0.0  # sample_sxyz()
    p.spin1y = 0.0  # sample_sxyz()
    ...
    p.spin2x = 0.0  # sample_sxyz()
    p.spin2y = 0.0  # sample_sxyz()
...
def reject_new_sample_point(
    new_point, points_table, MM, psd, f_min, dt, N, in_mchirp_window
):
...
while cnt < num_new_points:
    ...
    while reject_new_sample_point(
        new_point, new_points_table, MM, psd, f_min, dt, N, options.mchirp_window
    ):
