...
parser.add_option(
    "--bank-batch-size", help="No of bank points in each sub-job", type=int
)
...
if options.bank_batch_size:
    ...
    for batch in bank_batches:
        ...
        subbank_proctable = table.get_table(
            out_subbank_doc, lsctables.ProcessTable.tableName
        )
    ...
    bank_batches_kplusOne = [
        in_bank_table[i : i + k + 1] for i in range(NBk * k, len(in_bank_table), k + 1)
    ]

    # Just checking
    if (len(bank_batches_k) + len(bank_batches_kplusOne)) != NUM_SUB_BANKS or (
        NBk * k + NBkplusOne * kplusOne
    ) != len(in_bank_table):
