...
parser = OptionParser()
parser.add_option(
    "--subbank-file-name", help="The sub-bank file", dest="subbank_file_name"
)
parser.add_option(
    "--proposal-file-name", help="The new points file", dest="prop_file_name"
)
parser.add_option(
    "--match-file-name", help="The file to store matches", dest="match_file_name"
)
parser.add_option(
    "--subbank-batch-size",
...
def get_waveform(wav, f_min, dt, N):
    ...
    df = 1.0 / (dt * N)
    f_max = min(1.0 / (2.0 * dt), 0.15 / ((m1 + m2) * qm.QM_MTSUN_SI))

    htild = get_fd_waveform(
    ...
    href_padded = FrequencySeries(
        zeros(N / 2 + 1), delta_f=df, dtype=complex128, copy=True
    )
...
dt = 1.0 / np.float64(sample_rate)
N = signal_length * sample_rate
print(
    "f_min=%f, sig_len=%d, sample_rate=%d, dt=%f, N=%d"
    % (f_min, signal_length, sample_rate, dt, N)
)

# get the ZDHP psd
psd = pycbc.psd.from_asd_txt(
    "/home/prayush/advLIGO_PSDs/ZERO_DET_high_P.txt",
    N / 2 + 1,
    1.0 / np.float64(signal_length),
    f_min,
)
# psd *= DYN_RANGE_FAC **2
...
idx = 0
print(
    "subbank_batch_size = %d, Num of batches = %d"
...
if options.match_file_name:
    ...
    raise ValueError(
        "No Match file-name given to write the output for %s" % PROGRAM_NAME
    )
