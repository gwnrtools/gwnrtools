...
parser.add_option(
    "--unc-points-file",
    help="File containing mc-etai-chi values of uncoverde points, in THAT order in each column",
    type=str,
)
...
if options.unc_points_file is None or not os.path.exists(options.unc_points_file):
    raise ValueError(
        "The uncovered points file %s does not exist" % options.unc_points_file
    )
else:
