...
def make_z_cosmo_inverseCDF(z_max, R0, H0, Omega_m, Omega_Lambda, Omega_k, w0, w1):
    ...
    R_zmax = integrate.quad(
        dR_dz, 0, z_arr[-1], (H0, Omega_m, Omega_Lambda, Omega_k, w0, w1, R_0)
    )[0]
...
def probability_density_Uniform_comoving_volume(
    z, H0, Omega_m, Omega_Lambda, Omega_k, w0, w1, r0, R_zmax
):
    # eqn 11 in dP_L_M
...
def dR_dz(z, H0, Omega_m, Omega_Lambda, Omega_k, w0, w1, r0):
    # eqn 12 in dP_L_M
    e_of_z = 1.0  # (z, PARAMETERS) #assume this is constant across the
...
def dV_dz(z, H0, Omega_m, Omega_Lambda, Omega_k, w0, w1):
    ...
    return (
        4
        * np.pi
        * np.square(DL(z, H0, Omega_m, Omega_Lambda, Omega_k, w0, w1))
        / (
            np.square(1 + z)
            * H(z, H0, Omega_m, Omega_Lambda, Omega_k, w0, w1)
            / (lal.C_SI * 1e-3)
        )
    )
...
def DL(z, H0, Omega_m, Omega_Lambda, Omega_k, w0, w1):
    ...
    if Omega_k > 0.0:
        ...
        return ((lal.C_SI * 1e-3) * (1 + z) / np.sqrt(np.absolute(Omega_k))) * np.sin(
...
def Hubble_integral(z_prime, H0, Omega_m, Omega_Lambda, Omega_k, w0, w1):
    return integrate.quad(
        OneOverH, 0, z_prime, (H0, Omega_m, Omega_Lambda, Omega_k, w0, w1)
    )[0]
