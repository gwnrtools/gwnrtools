...
def convert_lalREAL8TimeSeries_to_TimeSeries(h):
    return TimeSeries(
        h.data.data, delta_t=h.deltaT, copy=True, epoch=h.epoch, dtype=h.data.data.dtype
    )
...
class nr_wave:
    ...
    def __init__(
        ...
        if "dataset" not in filetype:
            ...
            if verbose:
                print("\n Reading From Filename=%s" % filename, file=sys.stderr)
        ...
        if "HDF" in self.filetype:
            if self.verbose:
                print(
                    " > Reading NR data in HDF5 from %s" % self.filename,
                    file=sys.stderr,
                )
            ...
            for modeL in np.arange(2, self.modeLmax + 1):
                ...
                for modeM in np.arange(modeL, -1 * modeL - 1, -1):
                    ...
                    if self.verbose:
                        print("Reading %d,%d mode" % (modeL, modeM), file=sys.stderr)
                    #
                    ts = mdata[:, 0]
                    ...
                    self.rawmodes_real[modeL][modeM] = TimeSeries(
                        hp_int(self.rawtsamples[2][2]), delta_t=self.rawdelta_t, epoch=0
                    )
                    self.rawmodes_imag[modeL][modeM] = TimeSeries(
                        hc_int(self.rawtsamples[2][2]), delta_t=self.rawdelta_t, epoch=0
                    )
        #
            ...
            if rawdelta_t <= 0:
                ...
                self.rawdelta_t = rawdelta_t
            self.rawtsamples = TimeSeries(
                arange(0, ts.max()), delta_t=self.rawdelta_t, epoch=0
            )
            self.rawhp = TimeSeries(
                array([hp_int(t) for t in self.rawtsamples]),
                delta_t=self.rawdelta_t,
                epoch=0,
            )
            self.rawhc = TimeSeries(
                array([hc_int(t) for t in self.rawtsamples]),
                delta_t=self.rawdelta_t,
                epoch=0,
            )
            if self.verbose:
                print("times go from %f to %f" % (min(ts), max(ts)), file=sys.stderr)
                print(
                    "rawhp Min = %e, Max = %e" % (min(self.rawhp), max(self.rawhp)),
                    file=sys.stderr,
                )
            #
            ...
            if self.verbose:
                print(
                    "Reading NR data in ASCII from %s" % self.filename, file=sys.stderr
                )
            ...
            for k in list(f.keys()):
                ...
                try:
                    ...
                    try:
                        ...
                        if self.verbose:
                            print(
                                " .. Extrapolated groupname is %" % k, file=sys.stderr
                            )
                        raise IOError(
    ...
    def rescale_mode(self, M=None, distance=None, modeL=2, modeM=2):
        ...
        if self.verbose:
            print(
                " Rescaling mode %d, %d for M = %f" % (modeL, modeM, M), file=sys.stderr
            )
    ...
    def rescale_wave(self, M=None, inclination=None, phi=None, distance=None):
        ...
        amp22 = self.get_mode_amplitude(
            totalmass=self.totalmass, modeL=2, modeM=2, dimensionless=False
        )
            ...
            for modeM in np.arange(-1 * modeL, modeL + 1):
                ...
                if self.debug:
                    print(
                        ...
                        / np.pi,
                        (np.arctan2(curr_ylm.imag, curr_ylm.real) + (ArcTan2Merger))
                        / np.pi,
                    )
                    print(
                        (
                ...
                ##
                hp += (
                    self.rescaledmodes_real[modeL][modeM].data * curr_ylm.real
                    - self.rescaledmodes_imag[modeL][modeM].data * curr_ylm.imag
                )
                hc -= (
                    self.rescaledmodes_real[modeL][modeM].data * curr_ylm.imag
    ...
    def rescale_to_totalmass(self, M):
        ...
        return self.rescale_wave(
            M, inclination=self.inclination, phi=self.phi, distance=self.distance
        )
    ...
    def rotate(self, inclination=0, phi=0):
        ...
        return self.rescale_wave(
            self.totalmass, inclination=inclination, phi=phi, distance=self.distance
        )
    ...
    def get_polarizations(self, M=None, inclination=None, phi=None, distance=None):
        ...
        return self.rescale_wave(M, inclination=inclination, phi=phi, distance=distance)
    ...
    def get_frequency_t(self, t, totalmass=None):
        ...
        #
        NEWindex = 50
        mf = self.get_mode_frequency(
            dimensionless=False, startIdx=index - NEWindex, stopIdx=index + NEWindex
        )
        freq = (mf.data[NEWindex] + mf.data[NEWindex - 1]) / 2.0

        if self.verbose:
            print(
                "> get_orbital_frequency:: index = %d, freq = %f" % (index, freq),
                file=sys.stderr,
            )
    ...
    def get_peak_amplitude(self, amp=None, totalmass=None):
        ...
        iStart = int(len(amp.data) * 2.0 / 4.0)
    ...
    def taper_filter_waveform(
        ...
        **IMPORTANT** The domain of the window is passed as ttaper{1,2,3,4} values.
        ...
        if ttaper1 > ttaper2 or (ttaper1 + ttaper2 + ttaper4) > (
            len(hp0) * hp0.delta_t / self.totalmass / lal.MTSUN_SI
        ):
            raise IOError(
                "Invalid window configuration with [%f,%f,%f,%f]"
                % (ttaper1, ttaper2, ftaper3, ttaper4)
            )
        #
        hp = TimeSeries(
            hp0.data, dtype=hp0.dtype, delta_t=hp0.delta_t, epoch=hp0._epoch
        )
        hc = TimeSeries(
            hc0.data, dtype=hc0.dtype, delta_t=hc0.delta_t, epoch=hc0._epoch
        )
