...
def get_hplus_hcross_from_sxs(
    ...
    if debug:
        print(
            "mass, theta, phi , distance, end_time = ",
            total_mass,
            theta,
            phi,
            distance,
    ...
    while True:
        ...
        try:
            ...
            if isinstance(hdf5_file_name, UseNRinDA.nr_wave):
                ...
                if nrwav.dt != delta_t:
                    raise RuntimeError(
                        "sample_rate asked: %f, should be: %f"
                        % (1.0 / delta_t, 1.0 / nrwav.dt)
                    )
    ...
    if m_lower > total_mass:
        raise IOError(
            "Can rescale down to %f Msun at %fHz, asked for %f Msun"
    ...
    if debug:
        ...
        print(
            "\t Length of waveform BEFORE removing zeros = %d, %d" % (len(hp), len(hc))
        )
        sys.stdout.flush()
    ...
    time_start_s = (
        -nrwav.get_peak_amplitude(amplitude_from_polarizations(hp, hc))[0] * nrwav.dt
    )
    ...
    #
    hp = TimeSeries(
        hp.data, delta_t=delta_t, epoch=lal.LIGOTimeGPS(end_time + time_start_s)
    )
    hc = TimeSeries(
        hc.data, delta_t=delta_t, epoch=lal.LIGOTimeGPS(end_time + time_start_s)
    )
    #
    if debug:
        try:
