...
class BatchEvolutions(object):
    ...
    def check_exes(self, tests=None):
        ...
        for test in tests:
            ...
            if (
                config_name in self.exes.keys()
                and os.path.exists(self.exes[config_name])
                and os.path.getsize(self.exes[config_name]) > 0
            ):
    ...
    def setup_run(
        self, test, cluster="local", compiler="gcc", spectre_root=None, symlink_exe=True
    ):
        """
    ...
    def check_output(self, test, which=["volume", "reduction"]):
        ...
        for output in which:
            ...
            logging.info("Checking for {0:s} data...: {1:s}".format(output, out_file))
    ...
    def convert_volume_output_to_xdmf(self, test, conversion_bin=None):
        ...
        out_handler = HandleSpectreVolumeDatum(
            volume_data_file=out_file, name="TMP", read_fields=[], xdmf_converter=exe
        )
