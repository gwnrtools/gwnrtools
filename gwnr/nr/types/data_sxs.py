...
class nr_data:
        ...
        if "regex" in self.wavetype:
            ...
            for modeL in np.arange(2, self.modeLmax + 1):
                ...
                for modeM in np.arange(-1 * modeL, modeL + 1):
                    ...
                    self.modes[modeL][modeM] = nr_mode(
                        mdata, delta_t=self.delta_t, verbose=self.verbose
                    )
                ...
                if len(self.modes[modeL]) == 0:
                    self.modes.pop(modeL)
                ## If no (l) item exists in self.modes, no data has been read at ALL!
                if len(self.modes) == 0:
                    raise IOError("No (ASCII) data READ!")
        else:
            ...
            for kdx, k in enumerate(f.keys()):
                ...
                try:
                    ...
                    try:
                        ...
                        if self.verbose > 1:
                            print(
                                "\t\t.. tested (wavetype is extrapolated) groupname: {}".format(
                                    k
                                )
                            )
                        raise IOError(
    ...
    def read_nr_data_hdf5(self):
        ...
        if wavetype != "NoGroup":
            if self.verbose > 1:
                print(
                    "\tReading from {} out of ".format(group_name),
                    list(self.fin.keys()),
                )
