    ...
    def add_bilby_prior_files_configs(self):
        try:
            ...
            for f in self.bilby_prior_files:
                self.set(
                    "prior", f, "".join(open(self.bilby_prior_files[f]).readlines())
                )
        except:
...
""".format(
            ...
            self.set(
                "injection",
                "bbh-alignedspin-prior-{}".format(pt),
                """\
...
""".format(
            ...
            self.set(
                "injection",
                "bbh-eccentric-nonspin-prior-{}".format(pt),
                """\
...
class BilbyScriptWriterBase(object):
    def __init__(
        ...
        if "upper_frequency_cutoff" not in self.inference_opts:
            self.inference_opts["upper_frequency_cutoff"] = (
                self.inference_opts["sample_rate"] // 2
            )  # Nyquist
...
'''
        ...
        if self.source_type == "bns":
            imps.append("from bilby.gw.eos import TabularEOS, EOSFamily # for BNS")
...
template_generator = bilby.gw.WaveformGenerator(
    ...
    waveform_arguments=template_arguments)
            ...
            """.format(
                self.template_opts["approximant"],
    ...
    def add_prior_lines(self):
        if len(self.priors) == 0:
            ...
            if self.source_type == "bbh":
                ...
                raise IOError("Input `source_type` must be one of [bbh, bns, nsbh]")
        else:
            if self.verbosity > 0:
                print(
                    "Please do not forget to write '{0}'".format(self.priors_filename)
                )
            if self.source_type == "bbh":
                priors = [
                    "",
...
""".format(
                self.inference_opts["distance_marginalization"],
...
class BilbyScriptWriterInjection(BilbyScriptWriterBase):
    def __init__(
        ...
        for p in ["approximant", "source_model", "parameters"]:
            if p not in self.injection_opts:
                raise IOError("Please provide {} as part of `injection_opts`".format(p))
        self.set_default_injection_parameters()

        # Initialize
        super(BilbyScriptWriterInjection, self).__init__(
            tag,
            source_type,
    ...
    def set_default_injection_parameters(self):
        ...
        injection_parameter_defaults = dict(
            chi_1=0.0,
            chi_2=0.0,
            a_1=0.0,
            a_2=0.0,
        ...
        for p in injection_parameter_defaults:
            if (
                (p not in prececessing_spin_params and p not in aligned_spin_params)
                or (
                    p in prececessing_spin_params
                    and np.all(
                        [
                ...
                if p not in self.injection_opts["parameters"]:
                    self.injection_opts["parameters"][p] = injection_parameter_defaults[
                        p
                    ]
    ...
    def add_injection_lines(self):
        ...
        for p in self.injection_opts["parameters"]:
            injs.append("    {}={},".format(p, self.injection_opts["parameters"][p]))
        injs[-1] = injs[-1][:-1] + ")"
...
""".format(
                    self.injection_opts["approximant"],
...
""".format(
                    self.injection_opts["source_model"], self.ifo_list
                ),
            ]
        )
