...
def log_prior_enigma(
    ...
    if np.any(coeffs < -30.0) or np.any(coeffs > 30.0):
        if verbose:
            logging.info("Rejecting coeffs={} from prior for coeffs".format(coeffs))
        return -np.inf
    ...
    if omega_attach < float(sp_info.omega_attach.range[0]) or omega_attach > float(
        sp_info.omega_attach.range[-1]
    ):
        if verbose:
            logging.info(
    ...
    if len(sp_info.q.range) == 1:
        if q != float(sp_info.q.range[0]):
            if verbose:
                logging.info(
                    "Rejecting q={} from prior on q: {}".format(q, sp_info.q.range)
                )
            return -np.inf
    ...
    if len(sp_info.total_mass.range) == 1:
        ...
        if total_mass < float(sp_info.total_mass.range[0]) or total_mass > float(
    ...
    if verbose:
        logging.info(
            "ACCEPTED q={}, M={}, PNO={}, coeffs={}, om={}".format(
...
def log_likelihood_enigma(
    ...
    try:
        h1p, h1c = get_td_waveform(
            approximant="ENIGMA", mass1=mass1, mass2=mass2, f_lower=f_lower, delta_t=dt
        )
...
def log_prob_enigma(
    ...
    return (
        log_likelihood_enigma(
            ...
            inputs.sample_rate,
            psd,
            dilation_map_match=dilation_map_match,
        )
        + log_prior
    )
...
def log_prob_enigma_fixed_total_mass_hidden_q(
    ...
    hidden_params = all_params[
        all_params.columns[[all_params[c].vartype == "hidden" for c in all_params]]
    ]
