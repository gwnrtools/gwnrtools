...
def shift_waveform_phase_time(
    ...
    """
    hpnew = TimeSeries(
        hp, epoch=hp._epoch, delta_t=hp.delta_t, dtype=hp.dtype, copy=True
    )
    hcnew = TimeSeries(
        hc, epoch=hc._epoch, delta_t=hc.delta_t, dtype=hc.dtype, copy=True
    )
    # First apply phase shift
    if ph_shift != 0.0:
        amplitude = amplitude_from_polarizations(hpnew, hcnew)
        phase = phase_from_polarizations(hpnew, hcnew, remove_start_phase=False)
        if verbose:
    ...
    if t_shift != 0:
        if shift_epochs_only or t_shift <= 0:
            ...
            hpnew = TimeSeries(
                hp_shifted, epoch=hpnew._epoch + t_shift, delta_t=hpnew.delta_t
            )
            ...
            hcnew = TimeSeries(
                hc_shifted, epoch=hcnew._epoch + t_shift, delta_t=hcnew.delta_t
            )
...
def shift_waveform_phase(
    hp, hc, ph_shift, trim_leading=False, trim_trailing=True, verbose=False
):
    """
    Input:  hp, hc, where h = hp(t) + i hx(t) = Amp(t) * exp(-i * Phi(t))
    Output: hp, hc, where h = Amp(t) * exp( -i * [Phi(t) + phi_c] )
    """
    return shift_waveform_phase_time(
        hp,
        hc,
        0,
        ph_shift,
...
def align_waveforms_amplitude_peak(
    hplus1,
    hcross1,
    hplus2,
    hcross2,
    ...
    x0 = np.float64(amp1.sample_times[np.where(amp1.data == max(amp1.data))[0][0]])
    tmp = minimize_scalar(
        amp1I,
        x0,
        method="bounded",
    ...
    x0 = np.float64(amp2.sample_times[(np.where(amp2.data == max(amp2.data))[0][0])])
    tmp = minimize_scalar(
        amp2I,
        x0,
        method="bounded",
    ...
    if verbose:
        print(
            ("phase1 at peak idx = %d, = %f" % (int(np.round(t1 / hp1.delta_t)), ph1))
        )
        print(
            ("phase2 at peak idx = %d, = %f" % (int(np.round(t2 / hp2.delta_t)), ph2))
        )
    ...
    if shift_epochs_only:
        hp2, hc2 = shift_waveform_phase_time(
            hp2, hc2, t_shift, ph_shift, shift_epochs_only=True, verbose=verbose
        )
        ...
        if verbose:
            print(
                "phase shift is actually = {}, time shift = {}".format(
                    ph_shift, t_shift
                )
            )
        ...
        if t_shift >= 0:
            hp2, hc2 = shift_waveform_phase_time(
                hp2, hc2, t_shift, ph_shift, shift_epochs_only=False, verbose=verbose
            )
            hp1._epoch -= h1_max_amp_time
            hc1._epoch -= h1_max_amp_time
            ...
            hp1, hc1 = shift_waveform_phase_time(
                hp1,
                hc1,
                -1 * t_shift,
                ph_shift,
...
def align_waveforms_at_frequency(
    hplus1,
    hcross1,
    hplus2,
    hcross2,
...
def align_waveforms_optimally(
    ...
    if high_frequency_cutoff == None:
        high_frequency_cutoff = 1.0 / h_plus1.delta_t / 2.0
    #
    ...
    # >>>>>>
    while (
        np.abs(ph_shift) > phase_tolerance
        or np.abs(t_shift) > h_plus1.delta_t
        or np.abs(np.abs(olap.real) - optimal_overlap) > overlap_tolerance
    ):
        if idx == 0:
        ...
        snr, corr, snr_norm = matched_filter_core(
            h_plus1, hp2, psd, low_frequency_cutoff, high_frequency_cutoff, None
        )
        ...
        if len(h_plus1) > len(hp2):
            ...
            htilde = make_frequency_series(h_plus1)
            psd = from_string(
                psd_name, len(htilde), htilde.delta_f, low_frequency_cutoff
            )
    ...
    if verify:
        ...
        if (
            max_id != 0
            and max_id != 1
            and max_id != (len(snr) - 1)
            and max_id != (len(snr) - 2)
        ):
        ...
        if np.abs(ph_shift) > phase_tolerance:
            ...
            if verbose:
                print(
                    (
                        "Post-Alignmend Phase shift (should be < %.2e): %.2e"
                        % (phase_tolerance, np.abs(ph_shift))
                    )
                )
            print(
                ("Alignment in phasing correct.. (within tol %.2e)" % phase_tolerance)
            )
        #
...
def align_curves(
    ...
    for idx in range(num_retries):
        if verbose:
            ...
            sys.stdout.flush()
        retval = minimize_scalar(
            objective_function_alignment, args=opt_args, bounds=(x_min, x_max)
        )
        if objective_function_alignment(retval.x, *opt_args) < eps_solution:
            if verbose:
                print(
                    "optimization took {} objective func evals".format(
    ...
    raise RuntimeError(
        """Cannot solve the problem without either
...
def align_waveforms_suboptimally(
    ...
    h_plus1 = TimeSeries(
        hplus1, epoch=hplus1._epoch, delta_t=hplus1.delta_t, dtype=hplus1.dtype
    )
    h_cross1 = TimeSeries(
        hcross1, epoch=hplus1._epoch, delta_t=hplus1.delta_t, dtype=hplus1.dtype
    )
    h_plus2 = TimeSeries(
        hplus2, epoch=hplus2._epoch, delta_t=hplus2.delta_t, dtype=hplus2.dtype
    )
    h_cross2 = TimeSeries(
        hcross2, epoch=hplus2._epoch, delta_t=hplus2.delta_t, dtype=hplus2.dtype
    )
    #
    ...
    if t_shift >= 0:
        hp2, hc2 = shift_waveform_phase_time(
            h_plus2, h_cross2, tsign * t_shift, phsign * ph_shift, verbose=verbose
        )
    else:
        hp2, hc2 = shift_waveform_phase_time(
            h_plus2, h_cross2, tsign * t_shift, phsign * ph_shift, verbose=verbose
