...
def does_this_map(p, c, param="eta", return_err=False, verbose=False):
    ...
    if type(p) == dict:
        ...
        raise IOError("This type %s not supported by does_this_map" % str(type(p)))
    ...
    if type(c) == dict:
        ...
        raise IOError("This type %s not supported by does_this_map" % str(type(c)))
...
\n"""
    ...
    if "tmplt_bank_map" in method:
        ...
        indoc = ligolw_utils.load_filename(
            fp, contenthandler=LIGOLWContentHandler, verbose=verbose
        )
        #
        try:
        ...
        for idx, row in enumerate(fin):
            ...
            for param in params_tested:
                is_map = True
                tval, err = does_this_map(
                    p, row, param=param, return_err=True, verbose=verbose
                )
            ...
            if is_map:
                ...
                if verbose:
                    ...
                    print(
                        "Location of NR data     : %s " % getattr(row, "numrel_data"),
                        file=sys.stdout,
                    )
                #
        ...
        if len(mflower) > 0 and len(rows) > 0:
            if verbose:
                print(
                    " .. template matches %s simulations" % len(rows), file=sys.stderr
                )
            is_map = True
...
def get_hplus_hcross_from_sxs(
    ...
    if debug:
        print(
            "mass = {}, theta = {}, phi = {}, distance = {}, end_time = {}".format(
    ...
    if type(hdf5_file_name) != str:
        if verbose:
            print(
                "\tUsing nr_wave datastructure. Rescaling to {}Msun".format(total_mass)
            )
        ...
        while True:
            ...
            try:
                ...
                nrwav = gwnr.nr.nr_wave(
                    filename=hdf5_file_name,
                ...
                if num_length_tries == 0:
                    if verbose:
                        ...
                        print(
                            "Max number of length retries exceeded: {}. Final length tried: {}".format(
    ...
    m_lower = nrwav.get_lowest_binary_mass(t_filter, f_lower, totalmass=total_mass)
    if m_lower > total_mass:
        ...
        if verbose:
            print(
                "Can comfortably rescale to %f Msun starting at %fHz"
                % (total_mass, f_lower)
            )
    ...
    if taper:
        t_filter = t_start_M
        upwin_t_start = np.maximum(0.0, t_start_M - upwin_t_width)
        if upwin_t_width + upwin_t_start < junk_duration:
            if verbose > 0:
                print(
                    "Tapering window was not covering junk radiation. Changing it to [0,{}}]".format(
                        junk_duration
                    )
                )
            upwin_t_start = 0.0
        ...
        if verbose:
            print(
                "Length of waveform: ",
                (len(nrwav.rescaled_hp) * nrwav.delta_t / total_mass / lal.MTSUN_SI),
            )
        hp, hc = nrwav.taper_filter_waveform(
            ttaper1=upwin_t_start,
    ...
    if debug:
        print(
            "\t Length of waveform AFTER removing zeros = %d, %d" % (len(hp), len(hc))
        )
    #
    ...
    time_start_s = (
        -1
        * nrwav.get_amplitude_peak_h22(amplitude_from_polarizations(hp, hc))[-1]
        * nrwav.delta_t
    )
...
def get_hplus_hcross_from_get_td_waveform(**p):
    ...
    if "verbose" in list(p.keys()):
        ...
        verbose = False
    nr_data_location = get_nr_data_location(
        p, map_var="TemplateBankToNRMappings", method="tmplt_bank_map", verbose=verbose
    )
