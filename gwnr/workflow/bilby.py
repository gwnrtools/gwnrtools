...
from gwnr.stats.bilby_utils import BilbyScriptWriterEvent, BilbyScriptWriterInjection
from gwnr.workflow.inference import OneInferenceAnalysis, BatchInferenceAnalyses
    ...
    def get_injections_config(self):
        ...
        for subsec_name in self.get_subsections("injection"):
            ...
            injection_config_lines.append(
                "[{}]".format(subsec_name.replace("prior_", "prior-"))
            )
            if "static_params" in subsec_name:
                injection_config_lines.append(
                    "approximant = {}".format(get_injection_approximant(self))
                )

            sec_name = "injection-{}".format(subsec_name)
            for f in self[sec_name]:
                injection_config_lines.append("{} = {}".format(f, self[sec_name][f]))
...
class BilbyInferenceInjectionAnalysis(OneInferenceAnalysis):
    def __init__(self, opts, script_writer, run_dir, config_files, verbose=False):
        """
        ...
        super(BilbyInferenceInjectionAnalysis, self).__init__(
            opts,
            run_dir,
            config_files,
            inj_exe_name="",
    ...
    def setup(self):
        ...
        if self.verbose:
            logging.info(
                "Making {0} in {1}".format(self.get_analysis_dir(), os.getcwd())
            )
...
class BilbyInferenceEventAnalysis(OneInferenceAnalysis):
    def __init__(self, opts, script_writer, run_dir, config_files, verbose=False):
        ...
        """
        self.script_writer_obj = script_writer
        super(BilbyInferenceEventAnalysis, self).__init__(
            opts,
            run_dir,
            config_files,
            inj_exe_name="",
...
class BilbyOnInjectionBatch(BatchInferenceAnalyses):
    def __init__(self, opts, run_dir, config_files={}, verbose=False):
        ...
        """
        super(BilbyOnInjectionBatch, self).__init__(
            opts,
            run_dir,
            inj_exe_name="",
            inf_exe_name="",
        ...
        for p in var_inj_params:
            if p in static_inj_params:
                raise IOError(
                    "Inconsistent set of injection params specified. Check {}".format(p)
                )
        logging.info("--- injection params config verified")

        self.num_injections = opts.get_section_opts("injection", int)["num_injections"]

        logging.info("--- sampling {} injection params".format(self.num_injections))
        opts.write_injections_config()
        subprocess.check_output("rm -f injection.hdf", shell=True)
        subprocess.check_output(
            "pycbc_create_injections --config-files injection.ini --ninjections {} --output-file injection.hdf --force".format(
                self.num_injections
            ),
            shell=True,
        )
        ...
        for p in ["source_model", "approximant"]:
            if p not in injection_opts:
                logging.info(
                    "----- borrowing {} for injections from [template]".format(p)
                )
                injection_opts[p] = template_opts[p]
            if p not in template_opts:
                logging.info(
                    "--- borrowing {} for templates from [injection]".format(p)
                )
        ...
        for i in range(self.num_injections):
            ...
            logging.info("--- script writer object created for injection {}".format(i))

            run_tag = self.get_run_tag()
            self.runs[run_tag] = BilbyInferenceInjectionAnalysis(
                opts, obj, self.name_run_dir(i), config_files, verbose=self.verbose
            )
            logging.info("--- analysis objects created for injection {}".format(i))
    ...
    def name_run_dir(self, inj_num):
        return os.path.join(self.get_analyses_dir(), "injection{0:03d}".format(inj_num))
...
class BilbyOnEventBatch(BatchInferenceAnalyses):
    def __init__(self, opts, run_dir, config_files={}, verbose=False):
        ...
        run_dir : string
            Path to the main directory where all analyses are to be run
        """
        super(BilbyOnEventBatch, self).__init__(
            opts,
            run_dir,
            inj_exe_name="",
            inf_exe_name="",
        ...
        for i, event_name in enumerate(self.event_names):
            ...
            logging.info(
                "--- script writer object created for event {}".format(event_name)
            )
