...
class BanksimNode(CondorDAGNode):
    def __init__(
        ...
        if gpu and gpu_postscript:
            ...
            mf2 = match_file + ".1"
            self.add_file_opt(
                "match-file", match_file + ".$(Process)", file_is_output_file=True
            )
            self.job().__queue = 2
            ...
            job.add_condor_cmd(
                "Requirements", '(GPU_PRESENT =?= true) || (HasGPU =?= "gtx580")'
            )
...
class InferenceJob(CondorDAGJob, CondorJob):
    def __init__(
        ...
        self.add_condor_cmd("initialdir", os.path.dirname(os.path.abspath(executable)))

        if gpu:
            CondorJob.__init__(self, "vanilla", os.path.basename(executable), 2)
        # These are all python jobs so need to pull in the env
        self.add_condor_cmd("getenv", "True")
        log_base = os.path.join(
            log_dir, os.path.basename(executable) + "-$(cluster)-$(process)"
