...
class PycbcInferenceInjectionAnalysis(OneInferenceAnalysis):
    def __init__(
        ...
        """
        super(PycbcInferenceInjectionAnalysis, self).__init__(
            opts,
            run_dir,
            config_files,
    ...
    def setup(self):
        ...
        if self.verbose:
            logging.info(
                "Copying executables to {0}".format(
        ...
        for _, exe in self.opts.items("executables"):
            ...
            os.chmod(
                os.path.join(self.run_dir, "scripts/", os.path.basename(exe)), 0o0777
            )

        # Write injection creation script
        self.opts.set(self.inj_exe_name, "ninjections", "1")
        self.opts.set(self.inj_exe_name, "seed", str(numpy.random.randint(1, 1e5)))
        self.write_run_script(
            self.opts.items(self.inj_exe_name),
            "scripts/{0}".format(
                os.path.basename(self.opts.get("executables", self.inj_exe_name))
            ),
        ...
        self.write_run_script(
            self.opts.items(self.inf_exe_name),
            "scripts/{0}".format(
                os.path.basename(self.opts.get("executables", self.inf_exe_name))
            ),
...
class PycbcInferenceOnInjectionBatch(BatchInferenceAnalyses):
    def __init__(
        ...
        """
        super(PycbcInferenceOnInjectionBatch, self).__init__(
            opts,
            run_dir,
            inj_exe_name=inj_exe_name,
            inf_exe_name=inf_exe_name,
        ...
        self.config_combos = list(
            itertools.product(self.data_configs, self.sampler_configs, self.inf_configs)
        )
...
class PycbcInferenceEventAnalysis(OneInferenceAnalysis):
    def __init__(
        ...
        inf_exe_name : string
            Name of the inference exe's options' section in opts
        """
        super(PycbcInferenceEventAnalysis, self).__init__(
            opts,
            run_dir,
            config_files,
            inf_exe_name=inf_exe_name,
...
"""
            ...
            raise IOError(
                """The option psd-estimation in section workflow
    ...
    def fetch_all_psds(self, data_dir=None):
        ...
        if data_dir is None:
            data_dir = self.data_dir
        self.merger.fetch_psds(self.data_duration, self.data_sample_rate, data_dir)
    ...
    def setup(self):
        ...
        for ifo in self.merger.operating_ifos():
            myargs["{0}_frame_file".format(ifo)] = os.path.join(
                os.path.relpath(self.get_data_dir(), self.get_analysis_dir()),
                self.merger.frame_data_name(
                    ifo, self.data_duration, self.data_sample_rate
                ),
            )
...
class PycbcInferenceOnEventBatch(BatchInferenceAnalyses):
    def __init__(
        ...
        run_dir : string
            Path to the main directory where all analyses are to be run
        """
        super(PycbcInferenceOnEventBatch, self).__init__(
            opts,
            run_dir,
            inf_exe_name=inf_exe_name,
            plt_exe_name=plt_exe_name,
