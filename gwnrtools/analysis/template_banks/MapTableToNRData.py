...
def invert_tabletype(tabletype):
  if tabletype == lsctables.SnglInspiralTable: return lsctables.SimInspiralTable
  if tabletype == lsctables.SimInspiralTable: return lsctables.SnglInspiralTable
  raise IOError("Input table type neither Sim or Sngl Inspiral")

def new_row(tabletype):
  if tabletype == lsctables.SnglInspiralTable: return lsctables.SnglInspiral()
  if tabletype == lsctables.SimInspiralTable: return lsctables.SimInspiral()
  raise IOError("Input table type neither Sim or Sngl Inspiral")
...
if options.output_mapfile is not None:
  try: fout = h5py.File(options.output_mapfile, 'a')
  except:
    raise IOError("could not create OUTPUT MAP file %s" % options.output_mapfile)
  else:
    fout.create_group('TemplateBankToNRMappings-DataLocation')
    gout = fout['TemplateBankToNRMappings-DataLocation']
else:
  raise IOError("No file-name given for storing TEMPLATE to NR-DATA MAPPINGS")
...
if options.verbose:
  print >>sys.stdout, "EPS_Detault = ", EPS_Default, "\n", EPS_Params
  sys.stdout.flush()
...
for idx, tmplt in enumerate(input_bank):
  ...
  if is_map:
    ...
    #
    if options.verbose and idx % NOUTPUT == 0:
      print >>sys.stderr, " .. .. matched to %s" % mapstring
      print >>sys.stderr, " .. .. .. file exists: ", os.path.exists(mapstring)
    #
    ...
    errs = []
    for param in params_tested:
      _, err = does_this_map(tmplt, row, param=param, return_err=True, verbose=False)
      errs.append(err)
    #
    if options.verbose:
      print >>sys.stdout,\
...
if options.verbose:
  print >>sys.stdout, \
    "\n\n >> Total %d/%d templates mapped ..!" % (len(gout.keys()), len(input_bank))
  sys.stdout.flush()
