...
parser.add_option("--min-sz",help="lower spinZ bound",dest="min_sz",type=float,default=-0.9)
parser.add_option("--max-sz",help="upper spinZ bound", dest="max_sz", type=float, default=0.9)
...
# sampled.
sim_inspiral_table = lsctables.New(lsctables.SimInspiralTable,columns=['mass1','mass2','mchirp','eta','spin1x','spin1y','spin1z','spin2x','spin2y','spin2z','inclination','polarization','latitude','longitude','bandpass','alpha','alpha1','alpha2','process_id'])
...
for i in np.arange(npoints):
  ...
  smplpt.mchirp,smplpt.eta = qm.m1_m2_to_mchirp_eta(smplpt.mass1,smplpt.mass2)
  # Sample the spinZs  
  ...
  if options.eta is not None:
    ...
    smplpt.mass1, smplpt.mass2 = qm.mchirp_eta_to_m1_m2( smplpt.mchirp, smplpt.eta )
  ...
  while not accept_point_alignedspin( smplpt.mchirp/smplpt.eta**0.6, smplpt.eta, chi ):
    ...
    if (smplpt.spin1x or smplpt.spin1y or smplpt.spin1z) and not (options.spin1x or options.spin1y or options.spin1z):
    ...
    if (smplpt.spin2x or smplpt.spin2y or smplpt.spin2z) and not (options.spin2x is not None or options.spin2y is not None or options.spin2z is not None):
