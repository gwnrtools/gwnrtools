...
def get_waveform(approximant, phase_order, amplitude_order, template_params, start_frequency, sample_rate, length, filter_rate):
    ...
    if approximant in td_approximants():
        ...
        if True:
          print >>sys.stderr, "\n spin1z = %f, spin2z = %f, chi = %f" % (template_params.spin1z, template_params.spin2z, lalsimulation.SimIMRPhenomBComputeChi(template_params.mass1*lal.LAL_MSUN_SI, template_params.mass2*lal.LAL_MSUN_SI, template_params.spin1z, template_params.spin2z))
          print >>sys.stderr, "spin1x = %f, spin1y = %f" % (template_params.spin1x, template_params.spin1y)
          print >>sys.stderr, "spin2x = %f, spin2y = %f" % (template_params.spin2x, template_params.spin2y)
          print >>sys.stderr, "m1 = %f, m2 = %f" % (template_params.mass1, template_params.mass2)
        ...
        if True:
          print >>sys.stderr, "filter_N = %d in get waveform" % filter_N, "\n type(hvec) in get_waveform:", type(hvec)
	  print >>sys.stderr, hvec
	  print >>sys.stderr, hvec[0]
	  print >>sys.stderr, hvec[1]
	  print >>sys.stderr, isinstance(hvec,FrequencySeries)
