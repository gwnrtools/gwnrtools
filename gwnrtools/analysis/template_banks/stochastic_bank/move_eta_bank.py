...
parser.add_option("--extract-eta",help="single eta to be extracted",type=float)
...
parser.add_option("--strict-region", action="store_true", help="choose templates in a region, which has to be specified", default=False )
parser.add_option("--strict-region-alignedspin", action="store_true", help="choose templates in a region, which has to be specified. For BBH with chi1 = chi2.", default=False )
...
if options.columns is None:
	...
	if options.table == 'sim':
		out_subbank_table = lsctables.New(lsctables.SimInspiralTable,columns=['mass1','mass2','mchirp','eta','spin1x','spin1y','spin1z','spin2x','spin2y','spin2z','inclination','polarization','latitude','longitude','bandpass','alpha','alpha1','alpha2','process_id'])
	else:
		out_subbank_table = lsctables.New(lsctables.SnglInspiralTable,columns=['mass1','mass2','mchirp','eta','spin1x','spin1y','spin1z','spin2x','spin2y','spin2z','inclination','polarization','latitude','longitude','bandpass','alpha','alpha1','alpha2','process_id'])
else:
	cols = []
...
for bank_point in in_bank_table:
  ...
  if options.strict_region_alignedspin:
	...
	if options.extract_q is not None:
		options.extract_eta = options.extract_q / (1. +  options.extract_q)**2
