...
### option parsing ###
parser = OptionParser()
parser.add_option('-s', '--sim-file', metavar='FILE', help='file containing simulated injections', type=str)
parser.add_option('-b', '--bank-file', metavar='FILE', help='file containing simulated injections', type=str)
parser.add_option('-m', '--match-file',metavar='FILE', help='file containing the max over bank values for the injections')
...
while isfile(options.bank_file+str(index)+".xml"):
    indoc = ligolw_utils.load_filename(options.bank_file+str(index)+".xml", options.verbose)
...
for index in range(0,len(matches)) :
  ...
  itau0,itau3=qm.m1_m2_to_tau0_tau3(injected[index].mass1,injected[index].mass2,15)
  if hasattr(recovered[index],'mass1'):
      rtau0,rtau3=qm.m1_m2_to_tau0_tau3(recovered[index].mass1,recovered[index].mass2,15)
  else:
...
if not options.max_total_mass:
    pylab.figure(1)
    pylab.plot([min(inj_eta),max(inj_eta)],[min(inj_eta),max(inj_eta)],'-',color='black')
    ...
    pylab.figure(2)
    pylab.plot([min(inj_mchirp),max(inj_mchirp)],[min(inj_mchirp),max(inj_mchirp)],'-',color='black')
    ...
    n, bins,patches=pylab.hist(eta_err,bins=100,normed=True,histtype="stepfilled")
    pylab.savefig("hist_eta_err"+options.out_file)


    pylab.figure(4)
    pylab.xlabel('$\Delta \mathcal{M}_c/\mathcal{M}_c$')
    n, bins,patches=pylab.hist(mchirp_err,bins=100,normed=True,histtype="stepfilled")
...
if options.max_total_mass:
  ...
  pylab.figure(7)
  pylab.xlabel('Fractional Error in ($\eta$): ($\frac{\eta_injected - \eta_recovered}{\eta_injected}$)')
  pylab.ylabel('Number of Monte Carlo Points')
  n, bins,patches=pylab.hist(errE,bins=100,histtype="stepfilled")
  pylab.savefig("hist_eta_err"+str(options.max_total_mass)+options.out_file)


  pylab.figure(8)
  pylab.xlabel('Fractional Error in ($(\mathcal{M}_c): ($\frac{\mathcal{M}_injection - \matchcal{M}_recovered}{$\mathcal{M}_injected})')
  pylab.ylabel('Number of Monte Carlo Points')
  n, bins,patches=pylab.hist(errM,bins=100,histtype="stepfilled")
  pylab.savefig("hist_mchirp_err"+str(options.max_total_mass)+options.out_file)
