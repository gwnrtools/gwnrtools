...
parser = OptionParser(
    usage = "%prog [OPTIONS]",
    description = "Takes in the iteration id. It chooses num-new-points new points. Each of these points are chosen in a way that they have an overlap < MM with all other points. This is done to ensure that these new points are not overlapping in terms of the area that they cover.")

parser = OptionParser()
parser.add_option("--iteration-id",help="The index of the iteration",type=int,dest="iid")
parser.add_option("--num-new-points",help="No of bank points in each sub-job",default=1000,type=int)

parser.add_option('-m', '--minimal-match', metavar='MM', help='minimal match',default=0.95,type=float, dest="minimal_match")
...
parser.add_option('-w', '--mchirp-window', metavar='MC_WIN', help='Fractional window on mchirp parameter. If waveform parameters differ by more than this window, the overlap is set to 0.', default=0.01, type=float, dest="mchirp_window")
...
new_points_table = lsctables.New(lsctables.SimInspiralTable,columns=['mass1','mass2','mchirp','eta','spin1x','spin1y','spin1z','spin2x','spin2y','spin2z','inclination','polarization','latitude','longitude','bandpass','alpha','alpha1','alpha2','process_id'])
...
def reject_new_sample_point(new_point, points_table, MM, psd, f_min, dt, N, in_mchirp_window):
