...
parser.add_option("--bank-batch-size",help="No of bank points in each sub-job",type=int)
parser.add_option("--num-sub-banks",help="No of sub-banks=No of jobs",type=int)
...
if options.bank_batch_size and options.num_sub_banks:
  raise IOError("--bank-batch-size and --num-sub-banks cant BOTH be specified")

if not options.bank_batch_size and not options.num_sub_banks:
  raise IOError("ONE of --bank-batch-size and --num-sub-banks MUST be specified")
...
if options.bank_batch_size:
  ...
  bank_batches_kplusOne = [in_bank_table[i:i+k+1] for i in range(NBk*k, len(in_bank_table), k+1)]

  # Just checking
  if (len(bank_batches_k)+len(bank_batches_kplusOne)) != NUM_SUB_BANKS or (NBk*k + NBkplusOne*kplusOne) != len(in_bank_table):
