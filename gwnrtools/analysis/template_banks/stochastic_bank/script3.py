...
matplotlib.use('Agg')
import numpy as np
from numpy import sqrt, pi,cos, sin,loadtxt, float32,float64, complex128
...
parser = OptionParser(
    ...
    usage = "%prog [OPTIONS]",
    description = "Takes in a sub-bank file, and of proposal points. Computes overlaps between the systems in those files, and stores the maximum of these overlaps (for each proposal point) in a file match_id_part_pid.dat.")

parser = OptionParser()
parser.add_option("--subbank-file-name",help="The sub-bank file",dest="subbank_file_name")
parser.add_option("--proposal-file-name",help="The new points file",dest="prop_file_name")
parser.add_option("--match-file-name",help="The file to store matches",dest="match_file_name")
parser.add_option("--subbank-batch-size",help="No of bank points for which wavefors be pre-generated",default=500,type=int)

parser.add_option('-f', '--low-frequency-cutoff', metavar='FREQ', help='low frequency cutoff of matched filter', default=15.0, type=float, dest="f_min")
parser.add_option("-r", "--sample-rate",dest="sample_rate",help="Sample Rate [Hz]",default=4096,type=int)
parser.add_option("-l", "--signal-length",help="The length of the signal (s)",dest="signal_length",default=256,type=int)
...
def get_waveform(wav, f_min, dt, N):
  ...
  htild = get_fd_waveform(approximant="IMRPhenomC", mass1=m1, mass2=m2,
        spin1z=s1z, spin2z=s2z, f_lower=f_min, f_final=f_max, delta_f=df)
  
  htilde = FrequencySeries( htild, delta_f=df, dtype=complex128, copy=True )
  href_padded = FrequencySeries( zeros(N/2 + 1), delta_f=df, dtype=complex128, copy=True )
...
try:
  subbank_table = table.get_table(subbank_doc, lsctables.SimInspiralTable.tableName)
except ValueError:
  subbank_table = table.get_table(subbank_doc, lsctables.SnglInspiralTable.tableName)
...
try:
  prop_table = table.get_table(prop_doc, lsctables.SimInspiralTable.tableName)
except ValueError:
  prop_table = table.get_table(prop_doc, lsctables.SnglInspiralTable.tableName)
...
subbank_batches=[subbank_table[i:i+subbank_batch_size] for i in range(0, len(subbank_table), subbank_batch_size)]
...
for subbank_batch in subbank_batches:
  ...
  for prop_matches,prop_point in prop_point_matches:
    ...
    for stilde,subbank_point in subbank_sims:
      # Check
      if options.mchirp_window and outside_mchirp_window(prop_point,subbank_point,options.mchirp_window):
