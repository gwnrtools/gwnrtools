...
parser = OptionParser(
    ...
    usage = "%prog [OPTIONS]",
    description = "Takes in the old bank file, the proposals file, and the match file, and creates the new bank file by prepending the old bank file with points in the proposals file that have matches below the MM.")
...
parser.add_option('-m', '--minimal-match', metavar='MM', help='minimal match',default=0.95,type=float, dest="MM")
...
try :
    in_prop_table = table.get_table(in_prop_doc, lsctables.SimInspiralTable.tableName)
except ValueError:
    in_prop_table = table.get_table(in_prop_doc, lsctables.SnglInspiralTable.tableName)
...
if NUM_NEW_POINTS == 0:
  ...
  try:
    old_bank_table = table.get_table(old_bank_doc, lsctables.SimInspiralTable.tableName)
  except ValueError:
    old_bank_table = table.get_table(old_bank_doc, lsctables.SnglInspiralTable.tableName) 
  # Create the New bank file
  new_bank_doc = ligolw.Document()
  new_bank_doc.appendChild(ligolw.LIGO_LW())

  out_proc_id = ligolw_process.register_to_xmldoc(new_bank_doc,
  ...
  new_bank_table = lsctables.New(lsctables.SimInspiralTable,columns=['mass1','mass2','mchirp','eta','spin1x','spin1y','spin1z','spin2x','spin2y','spin2z','inclination','polarization','latitude','longitude','bandpass','alpha','alpha1','alpha2','process_id'])
  ...
  new_bank_proctable = table.get_table(new_bank_doc, lsctables.ProcessTable.tableName)
