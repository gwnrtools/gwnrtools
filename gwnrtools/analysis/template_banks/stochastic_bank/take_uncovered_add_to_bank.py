...
parser = OptionParser()
parser.add_option("--unc-points-file",help="File containing mc-etai-chi values of uncoverde points, in THAT order in each column",type=str)
parser.add_option("--etas-file",help="File containing allowed Eta values",type=str)
...
parser.add_option("--mchirp-cluster-window",help="Clustering Window on mchirp",type=float,default=0.)
parser.add_option("--first-chi", action="store_true", help="used if the uncovered point is placed based on chi", default=False )
parser.add_option("--first-eta", action="store_true", help="used if the uncovered point is placed based on eta", default=False )
...
if options.unc_points_file is None or not os.path.exists(options.unc_points_file):
  raise ValueError("The uncovered points file %s does not exist" % options.unc_points_file)
else:
