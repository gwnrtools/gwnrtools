...
class nr_wave():
    ...
    ### 2. ...
        ...
        if self.verbose:
            print("self.sample-rate & time_len = ", \
                    self.sample_rate, self.time_length, file=sys.stderr)
        if self.verbose: print("self.n = ", self.n, file=sys.stderr)
        ...
        if self.verbose:
            print(" >> Input mass, inc, phi, dist = ", totalmass,\
                                self.inclination, self.phi, self.distance, file=sys.stderr)
        ...
        if self.verbose: print(self.wavetype, file=sys.stderr)
        ...
        if 'HDF' in self.filetype:
            if self.verbose:
                print(" > Reading NR data in HDF5 from %s" % self.filename, file=sys.stderr)
            ...
            if self.wavetype != 'NoGroup':
                ...
                if self.verbose:
                    print(("From %s out of " % grp), list(self.fin.keys()), file=sys.stderr)
            ...
            for modeL in np.arange( 2, self.modeLmax+1 ):
                ...
                for modeM in np.arange( modeL, -1*modeL-1, -1 ):
                    ...
                    if self.verbose: print("Reading %d,%d mode" % (modeL, modeM), file=sys.stderr)
                    #
                    ts = mdata[:,0]
        ...
        #
            ...
            if self.verbose:
              ...
              print("rawhp Min = %e, Max = %e" % (min(self.rawhp), max(self.rawhp)), file=sys.stderr)
            #
        elif 'ASCII' in self.filetype:
            raise IOError("ASCII datafile not accepted yet!")
            if self.verbose: print("Reading NR data in ASCII from %s" % \
            ...
            for modeL in np.arange( 2, self.modeLmax+1 ):
                ...
                for modeM in np.arange( -1*modeL, modeL+1 ):
                    ...
                    if self.verbose: print(np.shape(mdata), file=sys.stderr)
                    #
                    ts = mdata[:,0]
                    hp_int = InterpolatedUnivariateSpline(ts, mdata[:,1])
                    hc_int = InterpolatedUnivariateSpline(ts, mdata[:,2])
    ...
    def get_groupname(self):
        #{{{
        if self.group_name is not None: return self.group_name
        ...
        #
            ...
            for k in list(f.keys()):
                if self.debug: print("Going to test: ", k)
                try: n = int(k[-1])
                except:
                    try: n = int(k.split('.dir')[0][-1])
                    except:
                        if self.verbose:
                            print(" .. Extrapolated groupname is %" % k, file=sys.stderr)
                        raise IOError("Could not find the group for extrapolated waveforms")
                if self.debug: print("Found n = ", n, " for ", k)
    ...
    def rescale_mode(self, M=None, distance=None, modeL=2, modeM=2):
        ...
        if self.verbose:
            print(" Rescaling mode %d, %d for M = %f" % (modeL,modeM, M), file=sys.stderr)
        ...
        if self.verbose: print(self.n, end_t_n, file=sys.stderr)
