...
class nr_wave():
    ...
    ### 2. ...
        ...
        if 'HDF' in self.filetype:
            if self.verbose:
                print(" > Reading NR data in HDF5 from %s" %
            ...
            if self.wavetype != 'NoGroup':
                ...
                if self.verbose:
                    print(("From %s out of " % grp), list(
                        self.fin.keys()), file=sys.stderr)
            ...
            for modeL in np.arange(2, self.modeLmax+1):
                ...
                for modeM in np.arange(modeL, -1*modeL-1, -1):
                    ...
                    if self.verbose:
                        print("Reading %d,%d mode" %
                              (modeL, modeM), file=sys.stderr)
                    #
                    ts = mdata[:, 0]
        ...
        #
            ...
            self.rawhc = TimeSeries(array([hc_int(t) for t in self.rawtsamples]),
                                    delta_t=self.rawdelta_t, epoch=0)
            if self.verbose:
                print("times go from %f to %f" %
                      (min(ts), max(ts)), file=sys.stderr)
                print("rawhp Min = %e, Max = %e" %
                      (min(self.rawhp), max(self.rawhp)), file=sys.stderr)
            #
        elif 'ASCII' in self.filetype:
            raise IOError("ASCII datafile not accepted yet!")
            if self.verbose:
                print("Reading NR data in ASCII from %s" %
                      self.filename, file=sys.stderr)
            ...
            for k in list(f.keys()):
                ...
                try:
                    ...
                    try:
                        ...
                        if self.verbose:
                            print(" .. Extrapolated groupname is %" %
                                  k, file=sys.stderr)
                        raise IOError(
    ...
    def rescale_mode(self, M=None, distance=None, modeL=2, modeM=2):
        ...
        if self.verbose:
            print(" Rescaling mode %d, %d for M = %f" %
                  (modeL, modeM, M), file=sys.stderr)
    ...
    def get_mode_amplitude(self, totalmass=None, modeL=2, modeM=2, dimensionless=False):
        ...
        if dimensionless:
            ...
            if totalmass is not None:
                hre, him = self.rescale_mode(
                    totalmass, modeL=modeL, modeM=modeM)
    ...
    def get_peak_amplitude(self, amp=None, totalmass=None):
        ...
        if amp is None:
            amp = self.get_mode_amplitude(
                totalmass=totalmass, dimensionless=False)
    ...
    def taper_filter_waveform(self, hpsamp=None, hcsamp=None, tapermethod='planck',
        ...
        **IMPORTANT** The domain of the window is passed as ttaper{1,2,3,4} values.
        ...
        #
        hp = TimeSeries(hp0.data, dtype=hp0.dtype,
                        delta_t=hp0.delta_t, epoch=hp0._epoch)
        hc = TimeSeries(hc0.data, dtype=hc0.dtype,
                        delta_t=hc0.delta_t, epoch=hc0._epoch)
        ...
        ntapers = np.array(
            [ntaper1, ntaper2, ntaper3, ntaper4], dtype=np.int64)
        ...
        if 'planck' in tapermethod:
            np.seterr(divide='raise', over='raise',
                      under='ignore', invalid='raise')
            t1, t2, t3, t4 = ttapers
            i1, i2, i3, i4 = ntapers
            if verbose:
                print("window times = ", t1, t2, t3,
                      t4, " idxs = ", i1, i2, i3, i4)
            #
